...
function loadCategoryBanner(categoryName) {
    ...
    const source =
        (category && (category.bannerUrl || category.imageUrl)) ||
        `https://via.placeholder.com/800x200/fbbf24/ffffff?text=${encodeURIComponent(categoryName)}`;
...
function updateBannerForSubcategory(subcategoryName) {
    ...
    let source = category.bannerUrl || category.imageUrl;
    
    if (subcategoryName && subcategoryName !== 'All Products' && category.subcategories) {
        const sub = category.subcategories.find(s => s.name === subcategoryName);
        if (sub) {
            source = sub.bannerUrl || sub.imageUrl || source;
        }
    }
    
    bannerImage.src = source || bannerImage.src;
}

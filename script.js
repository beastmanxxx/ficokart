...
function handleCategorySearchFocus() {
    ...
    if (modal) {
        ...
        // Scroll the main scrollable content of the category modal
        if (modalContent) {
...
// Redirect category modal search bar to global search section
window.redirectCategorySearchToGlobal = function() {
    const categoryInput = document.getElementById('categorySearchInput');
    const mainSearchInput = document.getElementById('mainSearchInput');

    // Capture current category context so back from search can reopen this modal
    const modal = document.getElementById('categoryModal');
    if (modal) {
        const categoryName = modal.getAttribute('data-category');
        const subcategory = modal.getAttribute('data-subcategory');
        if (categoryName) {
            lastCategorySearchContext = {
    ...
    if (categoryInput && mainSearchInput) {
        mainSearchInput.value = categoryInput.value || '';
    }

    // Close category modal visually (do NOT touch navigationStack here)
    if (modal && (modal.style.display === 'flex' || modal.style.display === 'block')) {
        closeCategoryModalInternal();
    }

    // Now go to global search section
    if (window.redirectToSearch) {
...
function setupCategoryModalScroll() {
    ...
    // Make sure search bar works even after scrolling by binding a single click handler
    if (!searchSection.dataset.searchRedirectBound) {
        searchSection.addEventListener('click', function(event) {
            // If user clicks anywhere inside the search section (input or icon), open global search
            if (window.redirectCategorySearchToGlobal) {
                window.redirectCategorySearchToGlobal();
            }
        });
        searchSection.dataset.searchRedirectBound = 'true';
    }

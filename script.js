...
window.adminCompleteOrder = async function (orderId) {
    try {
        ...
        // Transfer cashback from pending to earned when order is completed
        if (orderData && orderData.cashbackAmount > 0 && orderData.userId) {
            console.log(`Transferring ₹${orderData.cashbackAmount} cashback from pending to earned for order ${orderId}`);
    ...
    try {
        ...
        if (selectedPaymentMethod === 'online') {
            ...
            if (successfulOrders.length === cartItems.length) {
                // Calculate 1% combined cashback for all orders (based on original amount before wallet deduction)
                ...
                if (combinedCashbackAmount > 0) {
                    ...
                    // Add combined cashback directly to earned wallet (INSTANT)
                    const userRef = doc(db, 'users', currentUser.uid);
                    const userDoc = await getDoc(userRef);
                    const currentEarned = userDoc.exists() ? (userDoc.data().earnedCashback || 0) : 0;

                    await setDoc(userRef, {
                        earnedCashback: Math.round((currentEarned + combinedCashbackAmount) * 100) / 100
                    }, { merge: true });

                    console.log(`Instant cart cashback added: ₹${currentEarned} → ₹${currentEarned + combinedCashbackAmount}`);
                    ...
                    showNotification(`🎉 All orders placed successfully!<br>${successfulOrders.length} orders confirmed<br>Total amount: ${formatCurrency(totalAmount)}${walletMessage}<br>💰 Cashback: ${formatCurrency(combinedCashbackAmount)} added instantly!<br>Payment: Cash on Delivery`, 'success', 8000);
                } else {
                    const finalAmountPaid = Math.round((totalAmount - appliedWalletAmount) * 100) / 100;
                    const walletMessage = appliedWalletAmount > 0 ? `<br>💳 Wallet used: ${formatCurrency(appliedWalletAmount)}<br>💰 Final amount: ${formatCurrency(finalAmountPaid)}` : '';

                    showNotification(`🎉 All orders placed successfully!<br>${successfulOrders.length} orders confirmed<br>Total amount: ${formatCurrency(totalAmount)}${walletMessage}<br>Payment: Cash on Delivery`, 'success', 6000);
                }
    ...
    try {
        ...
        if (userDoc.exists()) {
            ...
            if (currentUser && userId === currentUser.uid) {
                console.log('🔄 Refreshing UI for current user');
...
function generateInvoiceHTML(order) {
    ...
    orderItems.forEach(item => {
        ...
        itemsHTML += `
            <tr>
                <td style="padding: 8px; border-bottom: 1px solid #eee; text-align: left; font-size: 15px;">
                    <div style="display: flex; align-items: center;">
                        ${productImageHTML}
                        <div>
                            <div>${item.name || item.productName || 'Product'}</div>
                            ${rating > 0 ? `<div style="color: #fbbf24; font-size: 13px; margin: 1px 0;">${'⭐'.repeat(rating)} (${rating})</div>` : ''}
                            <div style="color: #666; font-size: 13px;">
                                ${item.productSize || item.selectedSize ? `Size: ${item.productSize || item.selectedSize}` : ''}
                            ...
                            ${originalPrice > itemPrice && discount > 0 ? `<div style="color: #27ae60; font-size: 13px; font-weight: 600;">💰 Saved: ${formatCurrency((originalPrice - itemPrice) * itemQuantity)}</div>` : ''}
                        </div>
                    </div>
                </td>
                <td style="padding: 8px; border-bottom: 1px solid #eee; text-align: center; font-size: 15px;">${formatCurrency(itemPrice)}</td>
                <td style="padding: 8px; border-bottom: 1px solid #eee; text-align: center; font-size: 15px;">${itemQuantity}</td>
                <td style="padding: 8px; border-bottom: 1px solid #eee; text-align: right; font-weight: 600; font-size: 15px; color: #27ae60;">${formatCurrency(itemTotal)}</td>
            </tr>
        `;
    });
        ...
        <div style="background: white; padding: 20px; max-width: 600px; margin: 0 auto;">
            ...
            <div style="text-align: center; margin-bottom: 20px; border-bottom: 2px solid #3498db; padding-bottom: 15px;">
                <div style="display: flex; align-items: center; justify-content: center; gap: 12px;">
                    <img src="ficokartstamp.png" alt="Ficokart Stamp" style="width: 96px; height: 96px; object-fit: contain;" onerror="this.style.display='none'">
                    <div style="text-align: center;">
                        <div style="font-size: 32px; font-weight: 800; color: #000; letter-spacing: 1px;">Ficokart</div>
                        <h1 style="color: #2c3e50; margin: 0; font-size: 28px; font-weight: 700;">INVOICE BILL</h1>
                        <p style="color: #7f8c8d; margin: 5px 0 0 0; font-size: 16px;">Ficokart E-commerce</p>
                    </div>
                </div>
            </div>
            ...
            <div style="display: flex; justify-content: space-between; margin-bottom: 20px; flex-wrap: wrap;">
                <div style="flex: 1; width: 90vw;">
                    <h3 style="color: #2c3e50; margin: 0 0 8px 0; font-size: 18px;">Invoice Details</h3>
                    <p style="margin: 3px 0; color: #555; font-size: 15px;"><strong>Invoice #:</strong> INV-${order.id.substring(0, 8).toUpperCase()}</p>
                    <p style="margin: 3px 0; color: #555; font-size: 15px;"><strong>Order ID:</strong> ${order.id}</p>
                    <p style="margin: 3px 0; color: #555; font-size: 15px;"><strong>Date:</strong> ${orderDate}</p>
                </div>
                
                <div style="flex: 1; width: 90vw; text-align: right;">
                    <h3 style="color: #2c3e50; margin: 0 0 8px 0; font-size: 18px;">Customer Details</h3>
                    <p style="margin: 3px 0; color: #555; font-size: 15px;"><strong>Name:</strong> ${order.userName || 'N/A'}</p>
                    <p style="margin: 3px 0; color: #555; font-size: 15px;"><strong>Email:</strong> ${order.userEmail || 'N/A'}</p>
                    <p style="margin: 3px 0; color: #555; font-size: 15px;"><strong>Phone:</strong> ${order.userPhone || 'N/A'}</p>
                    <p style="margin: 3px 0; color: #555; font-size: 15px;"><strong>Address:</strong> ${order.userAddress || 'N/A'}</p>
                    <p style="margin: 3px 0; color: #555; font-size: 15px;"><strong>Payment:</strong> ${(() => {
            const method = order.paymentMethod || 'Not specified';
            if (method === 'cod') return 'Cash on Delivery (COD)';
...
async function handlePaymentSuccessForAllCartItems(paymentResponse) {
    try {
        ...
        if (successfulOrders.length > 0) {
            ...
            showNotification(`✅ Payment Successful! 🎉<br>${successfulOrders.length} orders placed<br>Total: ${formatCurrency(currentPaymentAmount)}<br>Payment ID: ${paymentResponse.razorpay_payment_id}${walletMessage}${cashbackMessage}`, 'success', 8000);

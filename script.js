        ...
        filteredUsers.forEach(user => {
            ...
            userItem.innerHTML = `
                <div class="user-info">
                    ...
                    <div class="user-wallet-admin" style="margin-top: 8px; padding: 6px; background: #fdf2f2; border-radius: 4px; border-left: 3px solid #ff4d4d; font-size: 11px;">
                        <span style="font-weight: 700; color: #d32f2f;">💰 WALLET:</span> 
                        <span style="margin-left: 5px;">Earned: <b>${formatCurrency(user.earnedCashback)}</b></span> | 
                        <span style="margin-left: 5px;">Pending: <b>${formatCurrency(user.pendingCashback)}</b></span>
                        <button onclick="editUserWallet('${user.uid}', '${user.email}', ${user.earnedCashback}, ${user.pendingCashback})" 
                                style="margin-left: 10px; padding: 2px 8px; font-size: 10px; cursor: pointer; background: #fff; border: 1px solid #d32f2f; border-radius: 3px; color: #d32f2f;">
                            ✏️ Edit Balance
                        </button>
                    </div>
                </div>
...
window.editUserWallet = async function (uid, email, currentEarned, currentPending) {
    const newEarned = prompt(`Update Earned Cashback for ${email}:`, currentEarned);
    if (newEarned === null) return;

    const newPending = prompt(`Update Pending Cashback for ${email}:`, currentPending);
    ...
    try {
        ...
        await updateDoc(doc(db, 'users', uid), {
            earnedCashback: earnedVal,
            pendingCashback: pendingVal
        });
...
async function loadWalletBalance() {
    ...
    try {
        ...
        if (userDoc.exists()) {
            ...
            document.getElementById('earnedCashback').textContent = formatCurrency(0);
            document.getElementById('pendingCashback').textContent = formatCurrency(0);
        }
    } catch (error) {
...
async function loadWalletBalanceInModal() {
    ...
    try {
        ...
        if (balanceElement) {
            balanceElement.textContent = formatCurrency(earnedCashback);
        }

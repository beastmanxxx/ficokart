...
import { getFirestore, collection, addDoc, getDocs, deleteDoc, doc, onSnapshot, setDoc, getDoc, updateDoc, query, where, orderBy, Timestamp, serverTimestamp } from 'https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js';
...
function getProductStatusBadgeHTML(product) {
    if (!product) return '';

    if (product.outOfStock) {
        return '<div class="product-status-badge" style="position: absolute; top: 0; left: 0; background: rgba(0,0,0,0.78); color: #fbbf24; padding: 4px 8px; border-radius: 0 0 8px 0; font-size: 10px; font-weight: 800; z-index: 6; text-transform: uppercase; letter-spacing: 0.4px;">Out of stock</div>';
    }

    if (product.comingSoon) {
        return '<div class="product-status-badge" style="position: absolute; top: 0; left: 0; background: rgba(0,0,0,0.78); color: #fbbf24; padding: 4px 8px; border-radius: 0 0 8px 0; font-size: 10px; font-weight: 800; z-index: 6; text-transform: uppercase; letter-spacing: 0.4px;">Coming soon</div>';
    }
...
// Custom Pricing helpers (Edit Product)
const CATEGORY_BANNER_SLIDE_DURATION = 4000;
let categoryBannerInterval = null;
let currentCategoryBannerIndex = 0;
let currentCategoryBannerImages = [];

function buildCategoryBannerPlaceholder(name) {
    return `https://via.placeholder.com/800x200/fbbf24/ffffff?text=${encodeURIComponent(name || 'Ficokart')}`;
}

function getCategoryBannerImagesFromData(category) {
    if (!category) return [];
    if (Array.isArray(category.bannerImages) && category.bannerImages.length > 0) {
        return category.bannerImages.filter(url => typeof url === 'string' && url.trim());
    }
...
function getSubcategoryBannerImagesFromData(subcategory) {
    if (!subcategory) return [];
    if (Array.isArray(subcategory.bannerImages) && subcategory.bannerImages.length > 0) {
        return subcategory.bannerImages.filter(url => typeof url === 'string' && url.trim());
    }
    if (subcategory.bannerUrl) return [subcategory.bannerUrl];
    if (subcategory.imageUrl) return [subcategory.imageUrl];
    return [];
}

function stopCategoryBannerAutoplay() {
    if (categoryBannerInterval) {
        clearInterval(categoryBannerInterval);
        categoryBannerInterval = null;
    }
}

function updateProductModalTitle(productId, title) {
    const titleEl = document.getElementById(`productTitle-${productId}`);
    if (!titleEl) return;

    const defaultTitle = titleEl.getAttribute('data-default-title') || titleEl.textContent;
    titleEl.textContent = title && title.trim() ? title.trim() : defaultTitle;
}

function startCategoryBannerAutoplay() {
    stopCategoryBannerAutoplay();
    if (currentCategoryBannerImages.length <= 1) return;
    categoryBannerInterval = setInterval(() => {
        setCategoryBannerSlide((currentCategoryBannerIndex + 1) % currentCategoryBannerImages.length);
    }, CATEGORY_BANNER_SLIDE_DURATION);
}

function updateCategoryBannerTransform() {
    const track = document.getElementById('categoryBannerTrack');
    if (track) {
        track.style.transform = `translateX(-${currentCategoryBannerIndex * 100}%)`;
    }
}

function renderCategoryBannerSlides(images = [], label = 'Category Banner') {
    const bannerSection = document.getElementById('categoryBannerSection');
    const track = document.getElementById('categoryBannerTrack');
    const slider = document.getElementById('categoryBannerSlider');
    if (!bannerSection || !track || !slider) return;

    const sanitizedImages = images.filter(url => typeof url === 'string' && url.trim());
    const slidesToRender = sanitizedImages.length > 0 ? sanitizedImages : [buildCategoryBannerPlaceholder(label)];

    currentCategoryBannerImages = slidesToRender;
    currentCategoryBannerIndex = 0;

    track.innerHTML = slidesToRender.map(url => `
        <div class="category-banner-slide">
            <img src="${url}" alt="${label}" onerror="this.src='${buildCategoryBannerPlaceholder(label)}'">
        </div>
    `).join('');

    bannerSection.classList.remove('collapsed');
    updateCategoryBannerTransform();
    setupCategoryBannerSwipe(slider);
    startCategoryBannerAutoplay();
}

window.setCategoryBannerSlide = function (index) {
    if (!currentCategoryBannerImages.length) return;
    const total = currentCategoryBannerImages.length;
    currentCategoryBannerIndex = ((index % total) + total) % total;
    updateCategoryBannerTransform();
    startCategoryBannerAutoplay();
};

function changeCategoryBannerSlide(direction) {
    if (!currentCategoryBannerImages.length) return;
    const total = currentCategoryBannerImages.length;
    setCategoryBannerSlide((currentCategoryBannerIndex + direction + total) % total);
}

function setupCategoryBannerSwipe(slider) {
    ...
    const onPointerEnd = (event) => {
        ...
        if (Math.abs(deltaX) > SWIPE_DISTANCE_THRESHOLD && elapsed < SWIPE_TIME_THRESHOLD) {
            changeCategoryBannerSlide(deltaX > 0 ? -1 : 1);
        } else {
            startCategoryBannerAutoplay();
        }
    ...
    slider.addEventListener('touchend', onPointerEnd, { passive: true });
    slider.addEventListener('touchcancel', onPointerCancel, { passive: true });
    slider.addEventListener('mousedown', onPointerStart);
    slider.addEventListener('mouseup', onPointerEnd);
    slider.addEventListener('mouseleave', onPointerCancel);

    slider.dataset.swipeBound = 'true';
}

function calculateEditCustomFinalPrice(input) {
    const row = input.closest('.edit-custom-pricing-row');
    if (!row) return;
    const priceInput = row.querySelector('.edit-custom-price');
    const discountInput = row.querySelector('.edit-custom-discount');
    const finalPriceEl = row.querySelector('.edit-custom-final-price');
...
function addEditCustomPricingRow() {
    const container = document.getElementById('editCustomPricingContainer');
    if (!container) return;
    const row = document.createElement('div');
    row.className = 'edit-custom-pricing-row';
    row.style.cssText = 'display: flex; align-items: center; gap: 8px; padding: 8px; border: 1px solid #bbf7d0; border-radius: 6px; background: white;';
    row.innerHTML = `
        <input type="text" class="edit-custom-name" placeholder="Custom Name (e.g., Family Pack)" style="flex: 1; padding: 8px; border: 1px solid #d1d5db; border-radius: 4px; font-size: 12px;">
        <input type="number" class="edit-custom-price" placeholder="Original Price (₹)" step="0.01" min="0" style="width: 120px; padding: 8px; border: 1px solid #d1d5db; border-radius: 4px; font-size: 12px;" oninput="calculateEditCustomFinalPrice(this)">
        <input type="number" class="edit-custom-discount" placeholder="Discount %" min="0" max="99" step="1" value="0" style="width: 80px; padding: 8px; border: 1px solid #facc15; border-radius: 4px; background: #fef9c3; font-size: 12px;" oninput="calculateEditCustomFinalPrice(this)">
        <div class="edit-custom-final-price" style="min-width: 80px; padding: 8px; background: #dcfce7; border: 1px solid #16a34a; border-radius: 4px; font-weight: 600; color: #16a34a; text-align: center; font-size: 12px;">₹0</div>
        <button type="button" onclick="removeEditCustomPricingRow(this)" style="background: #ef4444; color: white; border: none; padding: 6px 10px; border-radius: 4px; cursor: pointer; font-size: 10px;">Remove</button>
    `;
    container.appendChild(row);
}

function removeEditCustomPricingRow(button) {
    const row = button.closest('.edit-custom-pricing-row');
    if (!row) return;
    const container = row.parentElement;
    row.remove();
    // Ensure at least one row exists
    if (container && container.children.length === 0) {
        addEditCustomPricingRow();
    }
}

function loadEditCustomPricing(customPricing = []) {
    const container = document.getElementById('editCustomPricingContainer');
    if (!container) return;
    container.innerHTML = '';
    if (customPricing && customPricing.length > 0) {
        customPricing.forEach(option => {
            addEditCustomPricingRow();
            const row = container.lastElementChild;
            if (!row) return;
            const nameInput = row.querySelector('.edit-custom-name');
            const priceInput = row.querySelector('.edit-custom-price');
            const discountInput = row.querySelector('.edit-custom-discount');
            const finalPriceEl = row.querySelector('.edit-custom-final-price');
            if (nameInput) nameInput.value = option.name || '';
            if (priceInput) priceInput.value = option.originalPrice != null ? option.originalPrice : (option.price || '');
            if (discountInput) discountInput.value = option.discount != null ? option.discount : 0;
            if (finalPriceEl && priceInput) {
        ...
        // At least one empty row by default
        addEditCustomPricingRow();
    }
}
...
// Function to notify admins via email using our Node backend
async function notifyAdminsByEmail(orderData) {
    try {
        console.log('📬 Starting Admin Email notification process...');

        // 1. Get all admin emails from Firestore
        const adminEmails = [];
        const usersSnapshot = await getDocs(collection(db, 'users'));

        usersSnapshot.forEach((userDoc) => {
            const userData = userDoc.data();
            if (userData.isAdmin === true && userData.email) {
                adminEmails.push(userData.email);
            }
        });

        // 2. Add permanent admin from script if not in firebase
        ...
        console.log(`📨 Found ${adminEmails.length} admins. Notifying local backend...`);
        ...
        console.error('❌ Error in notifyAdminsByEmail:', error);
        // Important: We don't show an alert to the customer if the internal email notification fails
    }
}
...
window.loginWithEmailOrPhone = async function (emailOrPhone, password) {
    ...
    try {
        ...
        // Reset button state so modal can be reopened later without refresh
...
window.registerWithEmailAndPhone = async function (name, email, phone, password) {
    ...
    try {
        ...
        // Reset button so user can continue without forcing a reload
...
onAuthStateChanged(auth, (user) => {
    ...
    if (user) {
        ...
        // Start periodic check for pending cashback (every 30 seconds)
        setInterval(checkPendingCashbackStatus, 30000);
        console.log('🔄 Started periodic cashback status check (every 30 seconds)');
    } else {
...
function setupSlideshowSwipe(container) {
    ...
    container.addEventListener('touchcancel', onPointerCancel, { passive: true });
...
async function saveOrderToFirebase(orderData, skipCashback = false) {
    try {
        ...
        // For combined orders (with items array), different validation rules apply
        const isCombinedOrder = finalOrderData.items && Array.isArray(finalOrderData.items) && finalOrderData.items.length > 0;

        if (isCombinedOrder) {
            // For combined orders, only check these fields
            const combinedRequiredFields = ['totalAmount', 'userPhone', 'userAddress'];
            const missingFields = combinedRequiredFields.filter(field => !finalOrderData[field]);

            if (missingFields.length > 0) {
                console.error('Missing required fields for combined order:', missingFields);
                console.error('Order data:', finalOrderData);
                throw new Error(`Missing required fields: ${missingFields.join(', ')}`);
            }
        } else {
            // For single product orders, check all fields
        ...
        const cleanOrderData = {
            ...
            selectedSize: finalOrderData.selectedSize || finalOrderData.productSize || '',
            productSize: finalOrderData.productSize || finalOrderData.selectedSize || '',
            selectedColor: finalOrderData.selectedColor || finalOrderData.color || '', // Captured color
            color: finalOrderData.selectedColor || finalOrderData.color || '', // Backward compatibility
            quantity: finalOrderData.quantity || 1,
            items: finalOrderData.items || [], // Preserve items for cart orders
            totalAmount: finalOrderData.totalAmount || 0,
            userPhone: finalOrderData.userPhone || '',
            userAddress: finalOrderData.userAddress || '',
            googleMapsLink: finalOrderData.googleMapsLink || finalOrderData.userGoogleMapsLink || '',
            userGoogleMapsLink: finalOrderData.userGoogleMapsLink || finalOrderData.googleMapsLink || '',
            paymentMethod: finalOrderData.paymentMethod || 'cod',
            status: finalOrderData.status || 'Pending',
            orderType: finalOrderData.orderType || (finalOrderData.items ? 'Cart' : 'Single Product'),
        ...
        // Add ID to data for email notification
        const completeOrderData = { ...cleanOrderData, id: orderDocRef.id };

        // Admin Email Notification Call
        notifyAdminsByEmail(completeOrderData);

        // Calculate 1% cashback amount only if not skipping
        if (!skipCashback) {
            const cashbackBaseAmount = finalOrderData.originalAmount || finalOrderData.totalAmount;
            const cashbackPerHundred = 1; // 1%
            ...
            // Update order with cashback info - PENDING cashback
            if (cashbackAmount > 0) {
                ...
                // Add to pending cashback (will be earned after order completion)
                const userRef = doc(db, 'users', currentUser.uid);
                const userDoc = await getDoc(userRef);
                ...
                console.log(`Pending cashback added: ₹${currentPending} → ₹${currentPending + cashbackAmount}`);

                // Refresh wallet display to show pending cashback
                loadWalletBalance();
...
function displayFilteredOrders(orders) {
    ...
    orders.forEach(order => {
        ...
        const isCombinedOrder = order.items && Array.isArray(order.items) && order.items.length > 0;
        
        let headerHTML = '';
        if (isCombinedOrder) {
            // Combined order header
            headerHTML = `
                <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 8px; border-radius: 8px; margin-bottom: 8px; color: white;">
                    <div style="display: flex; align-items: center; gap: 8px;">
                        <div style="font-size: 18px;">🛒</div>
                        <div>
                            <div style="font-size: 12px; font-weight: 700;">Combined Cart Order</div>
                            <div style="font-size: 9px; opacity: 0.9;">${order.itemCount || order.items.length} items</div>
                        </div>
                    </div>
                </div>
                
                <!-- Items Summary (scrollable) -->
                <div style="max-height: 120px; overflow-y: auto; margin-bottom: 8px; padding-right: 3px;">
                    ${order.items.map((item, index) => {
                        const productImage = item.imageUrl || item.productImage;
                        const itemName = item.name || item.productName;
                        const itemPrice = item.price || item.productPrice;
                        const itemQty = item.quantity || 1;
                        const itemSize = item.selectedSize || item.productSize;
                        const itemColor = item.selectedColor || item.color;
                        const itemTotal = item.finalAmount || (itemPrice * itemQty);
                        const productId = item.productId || item.id;

                        return `
                            <div style="display: flex; gap: 6px; align-items: center; padding: 4px; background: #f8f9fa; border-radius: 4px; margin-bottom: 3px; cursor: pointer; transition: all 0.2s ease;" 
                                 onclick="openProductModal('${productId}')" 
                                 onmouseover="this.style.background='#e9ecef'" 
                                 onmouseout="this.style.background='#f8f9fa'">
                                <img src="${productImage}" alt="${itemName}" style="width: 25px; height: 25px; object-fit: contain; border-radius: 3px; background: white; border: 1px solid #e9ecef;" onerror="this.style.display='none'">
                                <div style="flex: 1; min-width: 0;">
                                    <div style="font-size: 10px; font-weight: 600; color: #2c3e50; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${itemName}</div>
                                    <div style="font-size: 8px; color: #666;">${itemSize ? `S:${itemSize}` : ''} ${itemColor ? `| C:${itemColor}` : ''}</div>
                                    <div style="font-size: 8px; color: #27ae60; font-weight: 600;">${formatCurrency(itemPrice)} x ${itemQty}</div>
                                </div>
                ...
                <div style="background: rgba(52, 152, 219, 0.08); border-radius: 8px; padding: 8px; margin-bottom: 6px; border-left: 3px solid #3498db;">
                    <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
                        ...
                        <div style="display: flex; align-items: center; gap: 4px;">
                            ...
                            <span style="font-size: 11px; color: #2c3e50; font-weight: 600;">Size: ${order.productSize}</span>
                        </div>
                        ` : ''}
                        ${(order.selectedColor || order.color) ? `
                        <div style="display: flex; align-items: center; gap: 4px;">
                            <span style="color: #9b59b6; font-size: 11px;">🎨</span>
                            <span style="font-size: 11px; color: #2c3e50; font-weight: 600;">Color: ${order.selectedColor || order.color}</span>
                        </div>
                        ` : ''}
                        <div style="display: flex; align-items: center; gap: 4px;">
                            <span style="color: #27ae60; font-size: 11px;">💰</span>
                            <span style="font-size: 10px; color: #27ae60; font-weight: 700;">${formatCurrency(order.totalAmount || 0)}</span>
                        </div>
                        ${order.walletUsed > 0 ? `
                        <div style="display: flex; align-items: center; gap: 4px;">
                            <span style="color: #f39c12; font-size: 11px;">💰</span>
                            <span style="font-size: 11px; color: #f39c12; font-weight: 600;">Wallet Applied: ${formatCurrency(order.walletUsed || 0)}</span>
                        </div>
                        ` : ''}
                        ${order.originalAmount && order.originalAmount !== order.totalAmount ? `
                        <div style="display: flex; align-items: center; gap: 4px;">
                            <span style="color: #95a5a6; font-size: 11px;">💸</span>
                            <span style="font-size: 10px; color: #95a5a6; text-decoration: line-through;">Original: ${formatCurrency(order.originalAmount || 0)}</span>
                        </div>
                        ` : ''}
                    </div>
...
window.adminCompleteOrder = async function (orderId) {
    try {
        ...
        // Schedule cashback transfer 24 hours after order completion
        if (orderData && orderData.cashbackAmount > 0 && orderData.userId) {
            console.log(`Scheduling cashback transfer of ₹${orderData.cashbackAmount} in 24 hours for order ${orderId}`);
            
            // Update order status to show cashback is scheduled
            await updateDoc(doc(db, 'orders', orderId), {
                cashbackStatus: 'scheduled',
                cashbackScheduledAt: new Date()
            }, { merge: true });
            
            // Schedule cashback transfer after 24 hours (86400000 ms)
            setTimeout(async () => {
                try {
                    console.log(`⏰ 24 hours elapsed - Transferring ₹${orderData.cashbackAmount} cashback from pending to earned for order ${orderId}`);
                    
                    // Update order status to completed
                    await updateDoc(doc(db, 'orders', orderId), {
                        cashbackStatus: 'earned',
                        cashbackEarnedAt: new Date()
                    }, { merge: true });
                    
                    // Transfer the cashback
                    ...
                    if (currentUser && currentUser.uid === orderData.userId) {
                        showNotification(`🎉 Cashback of ₹${orderData.cashbackAmount} is now available in your wallet!`, 'success');
                    }
                } catch (error) {
                    console.error('Error in scheduled cashback transfer:', error);
                }
            }, 86400000); // 24 hours delay
            
            // Show immediate notification to admin
            showNotification(`Order completed! ₹${orderData.cashbackAmount} cashback will be available to customer in 24 hours ⏰`, 'success');
        } else {
            showNotification('Order marked as Completed ✅', 'success');
        }

        // We no longer delete immediately or via setTimeout. 
        // Global cleanupOldOrders() takes care of this based on completedAt.
        // This ensures orders stay in the overview for the full 7 days.
...
window.adminRejectOrder = async function (orderId) {
    try {
        ...
        if (orderData && orderData.cashbackAmount > 0 && orderData.userId) {
            ...
            showNotification(`Order rejected! Customer's ${formatCurrency(orderData.cashbackAmount)} cashback cancelled ❌`, 'success');
        } else {
            showNotification('Order marked as Rejected ❌', 'success');
        }

        // We no longer delete immediately or via setTimeout. 
        // Global cleanupOldOrders() takes care of this based on rejectedAt.
...
async function addSlideToFirebase(title, url, linkedCategory = '') {
    try {
        ...
        if (linkedCategory) {
            slideData.linkedCategory = linkedCategory;
        }
...
async function loadCategoriesFromFirebase() {
    try {
        ...
        populateSlideCategoryDropdown(); // Keep slide category dropdown in sync
    } catch (error) {
...
async function addCategoryToFirebase(name, imageUrl, bannerUrls = [], subcategories = []) {
    try {
        const convertedUrl = convertGoogleDriveUrl(imageUrl);
        const normalizedBanners = Array.isArray(bannerUrls)
            ? bannerUrls.map(url => convertGoogleDriveUrl(url)).filter(url => !!url)
            : [];
        const primaryBanner = normalizedBanners[0] || '';
        console.log('Adding category:', name, convertedUrl, 'with banners:', normalizedBanners, 'and subcategories:', subcategories);
...
window.openCategoryModal = async function (categoryName, selectedSubcategory = null) {
    ...
    currentSelectedSubcategory = selectedSubcategory || null;
    ...
    // Set the selected subcategory if provided; otherwise default to null (show all)
    currentSelectedSubcategory = selectedSubcategory || null;

    const modal = document.getElementById('categoryModal');
    const title = document.getElementById('categoryModalTitle');
    title.textContent = categoryName;
    // Store context on modal so we can restore it after returning from search
    modal.setAttribute('data-category', categoryName);
    if (selectedSubcategory) {
        modal.setAttribute('data-subcategory', selectedSubcategory);
    } else {
        modal.removeAttribute('data-subcategory');
    }
...
window.openCategorySearchFromModal = function () {
    const modal = document.getElementById('categoryModal');
    if (!modal) return;

    const categoryName = modal.getAttribute('data-category');
    const subcategory = modal.getAttribute('data-subcategory');

    if (categoryName) {
        lastCategorySearchContext = { categoryName, subcategory: subcategory || null };
    }

    closeCategoryModalInternal();

    if (typeof window.redirectToSearch === 'function') {
        window.redirectToSearch();
    }
};
...
function closeCategoryModalInternal() {
    ...
    // Reset header and sidebar positions
...
function setupCategoryModalScroll() {
    ...
    if (!productsArea || !header || !sidebar || !bannerSection) return;

    productsArea.addEventListener('scroll', () => {
        if (productsArea.scrollTop > 80) {
            bannerSection.classList.add('collapsed');
        } else {
            bannerSection.classList.remove('collapsed');
        }
    });
...
function loadCategoryBanner(categoryName) {
    ...
    const images = getCategoryBannerImagesFromData(category);
    renderCategoryBannerSlides(images, categoryLabel);
}
...
function loadCategorySubcategories(categoryName) {
    ...
    const hasValidSelection = subcategories.some(sub => sub.name === currentSelectedSubcategory);
    ...
    subcategories.forEach(subcategory => {
        ...
        if (currentSelectedSubcategory && subcategory.name === currentSelectedSubcategory) {
...
function updateBannerForSubcategory(subcategoryName) {
    ...
    const isSubcategorySelected = Boolean(subcategoryName);
    let slideImages = [];
    let label = category.name || 'Category';

    if (isSubcategorySelected && category.subcategories) {
        const sub = category.subcategories.find(s => s.name === subcategoryName);
        if (sub) {
            slideImages = getSubcategoryBannerImagesFromData(sub);
            label = `${category.name || 'Category'} • ${sub.name}`;
        }
    }

    if (slideImages.length === 0) {
        slideImages = getCategoryBannerImagesFromData(category);
    }

    renderCategoryBannerSlides(slideImages, label);
}
...
function filterCategoryProducts() {
    ...
    // Filter by subcategory when one is selected
...
function displayCategoryProducts(products) {
    ...
    grid.innerHTML = products.map(product => {
        ...
        if (product.originalPrice && product.discount && product.originalPrice > product.price) {
            priceHTML = `
                <div class="category-product-price">
                    <span class="category-current-price">${formatCurrency(product.price)}</span>
                    <span class="category-original-price">${formatCurrency(product.originalPrice)}</span>
        ...
        const sizeBadges = hasSizes
            ? product.sizes.map(size => `<span class="size-badge" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 2px 6px; border-radius: 10px; font-size: 9px; font-weight: 600; display: inline-block;">${size}</span>`).join('')
            : '';
        const weightPricing = product.weightPricing || {};
        const hasWeightOptions =
            (weightPricing.ml && weightPricing.ml.length > 0) ||
            (weightPricing.g && weightPricing.g.length > 0) ||
            (weightPricing.kg && weightPricing.kg.length > 0);
        const sizeSection = `<div class="product-sizes" style="margin-bottom: 5px; display: flex; align-items: center; gap: 6px; flex-wrap: wrap;"><img src="mark.png" alt="Size" style="width: 16px; height: 16px; object-fit: contain;">${sizeBadges}</div>`;
        ...
        return `
            <div class="category-product-card" data-product-id='${product.id}' onclick="openProductModalFromCategory('${product.id}')">
                <div class="category-product-image-container">
                    ${getProductStatusBadgeHTML(product)}
...
window.searchCategoryProducts = function () {
    ...
    // Always reset scroll to top when user interacts with category search
    try {
        handleCategorySearchFocus();
    } catch (e) {
        console.warn('handleCategorySearchFocus not available', e);
    }
...
window.addCategory = async function () {
    ...
    const bannerUrl1 = document.getElementById('categoryBannerUrl1').value;
    const bannerUrl2 = document.getElementById('categoryBannerUrl2').value;
    const bannerUrl3 = document.getElementById('categoryBannerUrl3').value;
    const bannerUrls = [bannerUrl1, bannerUrl2, bannerUrl3]
        .map(url => url && url.trim())
        .filter(url => !!url);

    if (!name || !imageUrl || bannerUrls.length === 0) {
        alert('Please fill in category name, image, and at least one banner image URL');
        return;
    }

    const subcategories = collectSubcategories();

    const success = await addCategoryToFirebase(name, imageUrl, bannerUrls, subcategories);
    if (success) {
        loadCategoriesAdminData();
        // Clear form
        document.getElementById('categoryName').value = '';
        document.getElementById('categoryImageUrl').value = '';
        document.getElementById('categoryBannerUrl1').value = '';
        document.getElementById('categoryBannerUrl2').value = '';
        document.getElementById('categoryBannerUrl3').value = '';
...
window.addSubcategoryInput = function () {
    ...
    inputGroup.innerHTML = `
        ...
        <div class="subcategory-banner-inputs" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 8px;">
            <input type="url" class="subcategory-banner-input" data-banner-index="0" placeholder="Primary Banner Image URL" style="padding: 8px; border: 1px solid #ddd; border-radius: 5px;">
            <input type="url" class="subcategory-banner-input" data-banner-index="1" placeholder="Secondary Banner Image URL (optional)" style="padding: 8px; border: 1px solid #ddd; border-radius: 5px;">
            <input type="url" class="subcategory-banner-input" data-banner-index="2" placeholder="Tertiary Banner Image URL (optional)" style="padding: 8px; border: 1px solid #ddd; border-radius: 5px;">
        </div>
    `;
...
window.removeSubcategoryInput = function (button) {
    const inputGroup = button.closest('.subcategory-input-group');
    if (inputGroup) {
        inputGroup.remove();
    }
};
...
function collectSubcategories() {
    ...
    inputGroups.forEach(group => {
        ...
        const bannerInputs = group.querySelectorAll('.subcategory-banner-input');

        const name = nameInput.value.trim();
        const imageUrl = imageInput.value.trim();
        const bannerValues = Array.from(bannerInputs || []).map(input => input.value.trim()).filter(Boolean);
        const normalizedBanners = bannerValues.map(url => convertGoogleDriveUrl(url));

        if (name && imageUrl) {
            subcategories.push({
                name: name,
                imageUrl: convertGoogleDriveUrl(imageUrl),
                bannerUrl: normalizedBanners[0] || '',
                bannerImages: normalizedBanners
            });
        }
    });
...
function clearSubcategoriesForm() {
    ...
    if (inputGroups[0]) {
        ...
        inputGroups[0].querySelectorAll('.subcategory-banner-input').forEach(input => {
            input.value = '';
        });
    }
}
...
window.removeCategory = async function (categoryId) {
    if (!confirm('Are you sure you want to delete this category? All related data will be removed.')) {
        return;
    }
                ...
                <div style="margin-top: 10px;">
                    ...
                    <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                        ${category.subcategories.map((sub, subIndex) => `
                            <div style="display: flex; align-items: center; background: #f8f9fa; padding: 5px 10px; border-radius: 15px; border: 1px solid #e9ecef; gap: 6px;">
                                <span style="font-size: 10px; margin-right: 2px;">${sub.name}</span>
                                <button onclick="openEditSubcategoryModal('${category.id}', ${subIndex})" title="Edit subcategory" style="background: #f59e0b; color: white; border: none; border-radius: 50%; width: 20px; height: 20px; font-size: 10px; cursor: pointer; display: flex; align-items: center; justify-content: center;">✏️</button>
                                <button onclick="removeSubcategory('${category.id}', ${subIndex})" title="Remove subcategory" style="background: #dc3545; color: white; border: none; border-radius: 50%; width: 20px; height: 20px; font-size: 10px; cursor: pointer; display: flex; align-items: center; justify-content: center;">×</button>
                            </div>
                        `).join('')}
                    </div>
                </div>
...
let currentEditSubcategoryCategoryId = null;
let currentEditSubcategoryIndex = null;
...
window.addSubcategoryToCategory = function (categoryId) {
    ...
    document.querySelectorAll('.new-subcategory-banner').forEach(input => {
        input.value = '';
    });
};
...
window.openEditSubcategoryModal = function (categoryId, subIndex) {
    const category = categoriesData.find(cat => cat.id === categoryId);
    if (!category || !Array.isArray(category.subcategories) || !category.subcategories[subIndex]) {
        showNotification('Subcategory not found!', 'error');
        return;
    }

    currentEditSubcategoryCategoryId = categoryId;
    currentEditSubcategoryIndex = subIndex;

    const sub = category.subcategories[subIndex];
    const bannerInputs = document.querySelectorAll('.edit-subcategory-banner');
    const bannerImages = Array.isArray(sub.bannerImages) ? sub.bannerImages : (sub.bannerUrl ? [sub.bannerUrl] : []);

    document.getElementById('editSubcategoryName').value = sub.name || '';
    document.getElementById('editSubcategoryImage').value = sub.imageUrl || '';
    bannerInputs.forEach((input, idx) => {
        input.value = bannerImages[idx] || '';
    });
...
window.closeEditSubcategoryModal = function () {
    ...
    currentEditSubcategoryCategoryId = null;
    currentEditSubcategoryIndex = null;
    const form = document.getElementById('editSubcategoryForm');
    if (form) form.reset();
    document.querySelectorAll('.edit-subcategory-banner').forEach(input => input.value = '');
};
...
async function saveNewSubcategory() {
    ...
    const bannerInputs = document.querySelectorAll('.new-subcategory-banner');
    const bannerUrls = Array.from(bannerInputs).map(input => input.value.trim()).filter(Boolean);
    const normalizedBanners = bannerUrls.map(url => convertGoogleDriveUrl(url));
...
window.saveEditedSubcategory = async function () {
    if (!currentEditSubcategoryCategoryId || currentEditSubcategoryIndex === null) {
        showNotification('No subcategory selected for editing!', 'error');
        return;
    }

    const name = document.getElementById('editSubcategoryName').value.trim();
    const imageUrl = document.getElementById('editSubcategoryImage').value.trim();
    const bannerInputs = document.querySelectorAll('.edit-subcategory-banner');
    const bannerUrls = Array.from(bannerInputs).map(input => input.value.trim()).filter(Boolean);
    const normalizedBanners = bannerUrls.map(url => convertGoogleDriveUrl(url));

    if (!name) {
        showNotification('Please enter subcategory name!', 'error');
        return;
    }

    try {
        const category = categoriesData.find(cat => cat.id === currentEditSubcategoryCategoryId);
        if (!category || !Array.isArray(category.subcategories) || !category.subcategories[currentEditSubcategoryIndex]) {
            showNotification('Subcategory not found!', 'error');
            return;
        }

        const updatedSubcategories = [...category.subcategories];
        updatedSubcategories[currentEditSubcategoryIndex] = {
            ...updatedSubcategories[currentEditSubcategoryIndex],
            name,
        ...
        const categoryRef = doc(db, 'categories', currentEditSubcategoryCategoryId);
        await updateDoc(categoryRef, {
            subcategories: updatedSubcategories,
            updatedAt: new Date()
        });
...
window.editCategory = function (categoryId) {
    ...
    const bannerInputs = [
        document.getElementById('editCategoryBanner1'),
        document.getElementById('editCategoryBanner2'),
        document.getElementById('editCategoryBanner3')
    ];
    const existingBanners = getCategoryBannerImagesFromData(category);
    bannerInputs.forEach((input, idx) => {
        if (input) {
            input.value = existingBanners[idx] || '';
        }
    });
    ...
    productsData.forEach(product => {
        ...
        productItem.innerHTML = `
            ...
            <div style="flex: 1;">
                ...
                <div style="font-size: 10px; color: #666;">${formatCurrency(product.price)} • Rating: ${product.rating}⭐</div>
            </div>
        `;
...
window.saveEditedCategory = async function () {
    ...
    const bannerUrls = bannerInputs
        .map(input => input && input.value ? input.value.trim() : '')
        .filter(url => !!url);
    ...
    try {
        ...
        const updatedCategoryData = {
            ...
            bannerUrl: normalizedBanners[0] || currentCategory.bannerUrl || '',
            bannerImages: normalizedBanners.length > 0 ? normalizedBanners : (currentCategory.bannerImages || []),
...
async function loadProductsFromFirebase() {
    try {
        ...
        querySnapshot.forEach((doc) => {
            ...
            const normalizedWeight = normalizeWeightPricing(rawData.weightPricing);
            productsData.push({
                id: doc.id,
                ...rawData,
                weightPricing: normalizedWeight
            });
        });
        ...
        // Check for shared product after products are loaded
        checkAndOpenSharedProduct();
        ...
        // Check for shared product even in error case
        checkAndOpenSharedProduct();
...
// Flag to prevent duplicate shared product checks
let sharedProductChecked = false;

// Check and open shared product function
function checkAndOpenSharedProduct() {
    // Prevent multiple checks
    if (sharedProductChecked) return;
    sharedProductChecked = true;
    
    const urlParams = new URLSearchParams(window.location.search);
    const sharedProductId = urlParams.get('product');
    
    if (sharedProductId) {
        console.log('Found shared product ID:', sharedProductId);
        
        const product = productsData.find(p => p.id === sharedProductId);
        if (product) {
            console.log('Opening shared product:', product.name);
            console.log('Products available:', productsData.length);
            console.log('Calling openProductModal with ID:', sharedProductId);
            // Wait a bit for UI to be ready
            setTimeout(() => {
                console.log('Executing openProductModal...');
                window.openProductModal(sharedProductId);
            }, 500);
            
            // Clear the URL parameter after opening
            window.history.replaceState({}, document.title, window.location.pathname);
        } else {
            console.log('Product not found:', sharedProductId);
...
async function addProductToFirebase(name, imageUrl, description, finalPrice, subImages, categories, sizes, sizePricing, deliveryCharge, rating, originalPrice, discount, weightPricing, subcategories = [], sizeDiscounts = {}, keywords = '', isReturnable = true, customPricing = [], colorVariants = []) {
    try {
        const convertedUrl = convertGoogleDriveUrl(imageUrl);
        ...
        const productData = {
            ...
            isReturnable,
            customPricing,
            colors: colorVariants || [], // Added color variants correctly
...
function updateProductsDisplay() {
    ...
    displayProducts.forEach(product => {
        ...
        if (originalPrice && originalPrice > currentPrice && discount > 0) {
            priceHTML = `
                <div class="product-price-container">
                    ...
                    <div class="price-row" style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
                        <span class="current-price" style="color: #27ae60; font-size: 18px; font-weight: 700;">${formatCurrency(currentPrice)}</span>
                        <span class="original-price" style="color: #999; font-size: 14px; text-decoration: line-through;">${formatCurrency(originalPrice)}</span>
            ...
            priceHTML = `<div class="product-price" style="color: #27ae60; font-size: 18px; font-weight: 700;">${formatCurrency(currentPrice)}</div>`;
        }

        const hasSizes = product.sizes && product.sizes.length > 0;
        const sizeBadges = hasSizes
            ? product.sizes.map(size => `<span class="size-badge" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 3px 8px; border-radius: 12px; font-size: 10px; font-weight: 600; display: inline-block;">${size}</span>`).join('')
            : '';
        const weightPricing = product.weightPricing || {};
        const hasWeightOptions =
            (weightPricing.ml && weightPricing.ml.length > 0) ||
            (weightPricing.g && weightPricing.g.length > 0) ||
            (weightPricing.kg && weightPricing.kg.length > 0);
        const sizeSection = `<div class="product-sizes" style="margin-bottom: 6px; display: flex; align-items: center; gap: 8px; flex-wrap: wrap;"><img src="mark.png" alt="Size" style="width: 20px; height: 20px; object-fit: contain;">${sizeBadges}</div>`;
...
function collectAdditionalImages() {
    const container = document.getElementById('additionalImagesContainer');
    const additionalImageInputs = container ? container.querySelectorAll('.additional-image-url') : [];
    const imageUrls = [];
...
// Custom Pricing helpers (Add Product)
function calculateCustomFinalPrice(input) {
    const row = input.closest('.custom-pricing-row');
    if (!row) return;
    const priceInput = row.querySelector('.custom-price');
    const discountInput = row.querySelector('.custom-discount');
    const finalPriceEl = row.querySelector('.custom-final-price');
    const price = parseFloat(priceInput && priceInput.value ? priceInput.value : 0) || 0;
    const discount = parseFloat(discountInput && discountInput.value ? discountInput.value : 0) || 0;
    const finalPrice = price - (price * discount / 100);
    if (finalPriceEl) {
...
function addCustomPricingRow() {
    const container = document.getElementById('customPricingContainer');
    if (!container) return;
    const row = document.createElement('div');
    row.className = 'custom-pricing-row';
    row.style.cssText = 'display: flex; align-items: center; gap: 8px; padding: 8px; border: 1px solid #bbf7d0; border-radius: 6px; background: white;';
    row.innerHTML = `
        <input type="text" class="custom-name" placeholder="Custom Name (e.g., Family Pack)" style="flex: 1; padding: 8px; border: 1px solid #d1d5db; border-radius: 4px; font-size: 12px;">
        <input type="number" class="custom-price" placeholder="Original Price (₹)" step="0.01" min="0" style="width: 120px; padding: 8px; border: 1px solid #d1d5db; border-radius: 4px; font-size: 12px;" oninput="calculateCustomFinalPrice(this)">
        <input type="number" class="custom-discount" placeholder="Discount %" min="0" max="99" step="1" value="0" style="width: 80px; padding: 8px; border: 1px solid #facc15; border-radius: 4px; background: #fef9c3; font-size: 12px;" oninput="calculateCustomFinalPrice(this)">
        <div class="custom-final-price" style="min-width: 80px; padding: 8px; background: #dcfce7; border: 1px solid #16a34a; border-radius: 4px; font-weight: 600; color: #16a34a; text-align: center; font-size: 12px;">₹0</div>
        <button type="button" onclick="removeCustomPricingRow(this)" style="background: #ef4444; color: white; border: none; padding: 6px 10px; border-radius: 4px; cursor: pointer; font-size: 10px;">Remove</button>
    `;
    container.appendChild(row);
}

function removeCustomPricingRow(button) {
    const row = button.closest('.custom-pricing-row');
    if (!row) return;
    const container = row.parentElement;
    row.remove();
    // Ensure at least one row exists
...
window.resetProductForm = function () {
    ...
    document.getElementById('productFinalPrice').textContent = formatCurrency(0);
    ...
    const subSection = document.getElementById('productSubcategoriesSection');
    if (subSection) subSection.style.display = 'none';

    // Reset color variants
    ...
    // Reset custom pricing rows
    const customContainer = document.getElementById('customPricingContainer');
    if (customContainer) {
...
window.calculateMainProductFinalPrice = function () {
    ...
    if (originalPrice > 0) {
        ...
        document.getElementById('productFinalPrice').textContent = formatCurrency(finalPrice);
        document.getElementById('productFinalPriceValue').value = formatPrice(finalPrice);
    } else {
        document.getElementById('productFinalPrice').textContent = formatCurrency(0);
        document.getElementById('productFinalPriceValue').value = '0';
    }
...
window.calculateFinalPrice = function (size) {
    ...
    finalPriceDiv.textContent = formatCurrency(finalPrice);
};
...
window.addWeightInput = function (containerId, unit) {
    ...
    newWeightGroup.innerHTML = `
        ...
        <input type="url" class="weight-image" placeholder="Image URL (optional)" style="flex: 1; min-width: 150px; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">
        <input type="text" class="weight-title" placeholder="Display Title (optional)" style="flex: 1; min-width: 150px; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">
...
function collectWeightPricing() {
    ...
    // Helper to build one weight entry
    function buildEntry(group) {
        ...
        const discount = parseFloat(group.querySelector('.weight-discount').value) || 0;
        const imageInput = group.querySelector('.weight-image');
        const imageUrl = imageInput && imageInput.value ? convertGoogleDriveUrl(imageInput.value.trim()) : '';
        const titleInput = group.querySelector('.weight-title');
        ...
        return {
            ...
            price: originalPrice, // original (for reference)
            originalPrice: originalPrice, // always the un-discounted price
            finalPrice: finalPrice,    // pre-computed discounted price
            discount,
            imageUrl,
            title
        };
    ...
    document.getElementById('mlContainer').querySelectorAll('.weight-input-group').forEach(g => {
        const entry = buildEntry(g); if (entry) weightPricing.ml.push(entry);
    });
    document.getElementById('gContainer').querySelectorAll('.weight-input-group').forEach(g => {
        const entry = buildEntry(g); if (entry) weightPricing.g.push(entry);
    });
    document.getElementById('kgContainer').querySelectorAll('.weight-input-group').forEach(g => {
        const entry = buildEntry(g); if (entry) weightPricing.kg.push(entry);
    });
...
function normalizeWeightPricing(weightPricing = {}) {
    const normalized = { ml: [], g: [], kg: [] };
    ['ml', 'g', 'kg'].forEach(unit => {
        const options = Array.isArray(weightPricing[unit]) ? weightPricing[unit] : [];
        normalized[unit] = options.filter(option => {
            if (!option) return false;
            const quantity = typeof option.quantity === 'string' ? option.quantity.trim() : option.quantity;
            const price = option.price ?? option.originalPrice;
            return Boolean(quantity) && price !== undefined && price !== null && !Number.isNaN(parseFloat(price));
        }).map(option => ({
            quantity: typeof option.quantity === 'string' ? option.quantity.trim() : option.quantity,
            price: parseFloat(option.price ?? option.originalPrice),
            originalPrice: option.originalPrice !== undefined ? parseFloat(option.originalPrice) : parseFloat(option.price ?? option.originalPrice),
            discount: option.discount ? parseFloat(option.discount) : 0,
            imageUrl: option.imageUrl || '',
            title: option.title || option.displayTitle || ''
        }));
    });
    return normalized;
}
...
window.addProduct = async function () {
    ...
    // Get custom pricing entries
    const customPricing = [];
    const customContainer = document.getElementById('customPricingContainer');
    if (customContainer) {
        const rows = customContainer.querySelectorAll('.custom-pricing-row');
        rows.forEach(row => {
            ...
            if (nameVal && priceVal) {
                ...
                const finalPriceNum = priceNum - (priceNum * discountNum / 100);
                customPricing.push({
                    name: nameVal,
    ...
    // Get color variants
    const colorVariants = collectColorVariants();
    ...
    if (await addProductToFirebase(name, imageUrl, description, finalPrice, subImages, selectedCategories, sizes, sizePricing, deliveryCharge, rating, originalPrice, discount, weightPricing, selectedSubcategories, sizeDiscounts, keywords, isReturnable, customPricing, colorVariants)) {
...
// --- Color Variant Admin Logic ---

window.addColorVariantInput = function () {
    const container = document.getElementById('productColorsContainer');
    const colorGroup = document.createElement('div');
    colorGroup.className = 'color-variant-group';
    colorGroup.style.cssText = 'padding: 15px; border: 1px solid #ddd; border-radius: 8px; background: white; margin-bottom: 15px;';

    colorGroup.innerHTML = `
        <div style="display: flex; gap: 10px; align-items: center; margin-bottom: 10px;">
            <input type="text" class="color-name" placeholder="Color Name (e.g. Red, Blue)" style="flex: 1; padding: 10px; border: 1px solid #ddd; border-radius: 5px;">
            <button type="button" onclick="removeColorVariantInput(this)" style="background: #e74c3c; color: white; border: none; padding: 10px; border-radius: 5px; cursor: pointer;">Remove Color</button>
        </div>
        <div class="color-images-container" style="padding: 10px; background: #f8f9fa; border-radius: 5px;">
            <label style="display: block; font-size: 12px; font-weight: 600; margin-bottom: 5px;">Color Specific Images:</label>
            <div class="color-image-inputs" style="display: flex; flex-wrap: wrap; gap: 8px;">
                <input type="url" class="color-image-url" placeholder="Image URL" style="flex: 1; min-width: 200px; padding: 8px; border: 1px solid #ddd; border-radius: 5px; margin-bottom: 5px;">
            </div>
            <button type="button" onclick="addColorImageInput(this)" style="margin-top: 5px; background: #27ae60; color: white; border: none; padding: 5px 10px; border-radius: 5px; cursor: pointer; font-size: 11px;">+ Add Image</button>
        </div>
    `;
    container.appendChild(colorGroup);
};

window.removeColorVariantInput = function (button) {
    const parent = button.closest('.color-variant-group') || button.closest('.edit-color-variant-group');
    if (parent) parent.remove();
};

window.addColorImageInput = function (button) {
    const container = button.previousElementSibling;
    const input = document.createElement('input');
    input.type = 'url';
    input.className = 'color-image-url';
    input.placeholder = 'Image URL';
    input.style.cssText = 'flex: 1; min-width: 200px; padding: 8px; border: 1px solid #ddd; border-radius: 5px; margin-bottom: 5px;';
    container.appendChild(input);
};

// Edit modal versions
window.addEditColorVariantInput = function (name = '', images = []) {
    const container = document.getElementById('editProductColorsContainer');
    const colorGroup = document.createElement('div');
    colorGroup.className = 'edit-color-variant-group';
    colorGroup.style.cssText = 'padding: 15px; border: 1px solid #ddd; border-radius: 8px; background: white; margin-bottom: 15px;';

    let imagesHtml = '';
    if (images.length > 0) {
        images.forEach(img => {
            imagesHtml += `<input type="url" class="edit-color-image-url" value="${img}" placeholder="Image URL" style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 5px; margin-bottom: 5px;">`;
        });
    } else {
        imagesHtml = `<input type="url" class="edit-color-image-url" placeholder="Image URL" style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 5px; margin-bottom: 5px;">`;
    }

    colorGroup.innerHTML = `
        <div style="display: flex; gap: 10px; align-items: center; margin-bottom: 10px;">
            <input type="text" class="edit-color-name" value="${name}" placeholder="Color Name" style="flex: 1; padding: 10px; border: 1px solid #ddd; border-radius: 5px;">
            <button type="button" onclick="removeColorVariantInput(this)" style="background: #e74c3c; color: white; border: none; padding: 10px; border-radius: 5px; cursor: pointer;">Remove Color</button>
        </div>
        <div class="color-images-container" style="padding: 10px; background: #f8f9fa; border-radius: 5px;">
            <div class="edit-color-image-inputs" style="display: flex; flex-wrap: wrap; gap: 8px;">
                ${imagesHtml}
            </div>
            <button type="button" onclick="addEditColorImageInput(this)" style="margin-top: 5px; background: #27ae60; color: white; border: none; padding: 5px 10px; border-radius: 5px; cursor: pointer; font-size: 11px;">+ Add Image</button>
        </div>
    `;
    container.appendChild(colorGroup);
};

window.addEditColorImageInput = function (button) {
    const container = button.previousElementSibling;
    const input = document.createElement('input');
    input.type = 'url';
    input.className = 'edit-color-image-url';
    input.placeholder = 'Image URL';
    input.style.cssText = 'flex: 1; min-width: 200px; padding: 8px; border: 1px solid #ddd; border-radius: 5px; margin-bottom: 5px;';
    container.appendChild(input);
};

function collectColorVariants(isEdit = false) {
    const prefix = isEdit ? 'edit-' : '';
    const containerId = isEdit ? 'editProductColorsContainer' : 'productColorsContainer';
    const groupClass = isEdit ? 'edit-color-variant-group' : 'color-variant-group';
    const nameClass = isEdit ? 'edit-color-name' : 'color-name';
    const imageClass = isEdit ? 'edit-color-image-url' : 'color-image-url';

    const variants = [];
    const groups = document.querySelectorAll(`.${groupClass}`);

    groups.forEach(group => {
        const name = group.querySelector(`.${nameClass}`).value.trim();
        const imageInputs = group.querySelectorAll(`.${imageClass}`);
        const images = Array.from(imageInputs).map(input => input.value.trim()).filter(url => url !== '');

        if (name && images.length > 0) {
            variants.push({ colorName: name, images: images });
        }
    });
...
function loadProductsAdminData() {
    ...
    filteredProducts.forEach(product => {
        ...
        const originalPriceText = product.originalPrice && product.originalPrice !== product.price ? `<span style="text-decoration: line-through; color: #999; font-size: 11px;">${formatCurrency(product.originalPrice)}</span> ` : '';
        ...
        productCard.innerHTML = `
            ...
            <div class="product-info">
                ...
                <div style="font-size: 13px; color: #059669; font-weight: bold; margin-bottom: 2px;">
                    ${originalPriceText}<span style="color: #059669;">${formatCurrency(product.price)}</span>${discountText}
                </div>
                <div style="font-size: 11px; color: #666; margin-bottom: 4px;">Category: ${product.category || 'No category'}</div>
                ${product.colors && product.colors.length > 0 ? `<div style="margin-bottom: 4px; font-size: 10px; color: #8e44ad;">🎨 ${product.colors.length} Color Variants</div>` : ''}
...
window.openEditProductModal = function (productId) {
    ...
    const comingSoonEl = document.getElementById('editProductComingSoon');
    if (comingSoonEl) comingSoonEl.checked = !!product.comingSoon;
    const outOfStockEl = document.getElementById('editProductOutOfStock');
    if (outOfStockEl) outOfStockEl.checked = !!product.outOfStock;
    const editStatusEl = document.getElementById('editProductIsNew');
    if (editStatusEl) editStatusEl.checked = !!product.isNew;
    ...
    const editColorsContainer = document.getElementById('editProductColorsContainer');
    if (editColorsContainer) {
        editColorsContainer.innerHTML = '';
        if (product.colors && product.colors.length > 0) {
            product.colors.forEach(color => {
                addEditColorVariantInput(color.colorName, color.images);
            });
        }
    }

    // Populate weight pricing
    // Weight pricing will be populated by loadEditWeightData call below
    ...
    loadEditSizes(product.sizes || [], product.sizePricing || {}, product.sizeDiscounts || {}, product.variantStatus || {});

    // Load and populate weight-based pricing data
    loadEditWeightData(product.weightPricing || {}, product.variantStatus || {});

    // Load custom pricing data
    loadEditCustomPricing(product.customPricing || []);
...
window.calculateEditProductFinalPrice = function () {
    ...
    document.getElementById('editProductFinalPrice').textContent = formatCurrency(finalPrice);
    document.getElementById('editProductFinalPriceValue').value = formatPrice(finalPrice);
};
...
window.loadEditSizes = function (selectedSizes = [], sizePricing = {}, sizeDiscounts = {}, variantStatus = {}) {
    console.log('Loading edit sizes:', { selectedSizes, sizePricing });

    // First, uncheck and reset all size-related fields to ensure a clean state
    const allEditCheckboxes = document.querySelectorAll('.edit-size-checkbox');
    allEditCheckboxes.forEach(cb => {
        ...
        const comingSoonCb = document.getElementById(`editComingSoon${size}`);
        const outOfStockCb = document.getElementById(`editOutOfStock${size}`);
        const finalPriceEl = document.getElementById(`editFinalPrice${size}`);

        if (priceInput) priceInput.value = '';
        if (discountInput) discountInput.value = '0';
        if (comingSoonCb) comingSoonCb.checked = false;
        if (outOfStockCb) outOfStockCb.checked = false;
        if (finalPriceEl) finalPriceEl.textContent = '₹0';
    });

    // If selectedSizes is empty, try to get sizes from sizePricing keys
    let sizesToLoad = [...selectedSizes];
    if (sizesToLoad.length === 0 && sizePricing && Object.keys(sizePricing).length > 0) {
        sizesToLoad = Object.keys(sizePricing);
        console.log('Using sizePricing keys for recovery:', sizesToLoad);
    }

    // Now check selected sizes and populate pricing
    sizesToLoad.forEach(size => {
        if (!size) return;

        // Find checkbox by ID first, then by value as fallback
        const checkbox = document.getElementById(`edit-size-${size}`) ||
            document.querySelector(`.edit-size-checkbox[value="${size}"]`);
        ...
        if (checkbox) {
            ...
            if (priceInput && sizePricing[size] !== undefined) {
                priceInput.value = sizePricing[size];
            }
            if (discountInput && sizeDiscounts[size] !== undefined) {
                discountInput.value = sizeDiscounts[size];
            }

            if (variantStatus && variantStatus[size]) {
                if (comingSoonCb) comingSoonCb.checked = variantStatus[size].comingSoon || false;
                if (outOfStockCb) outOfStockCb.checked = variantStatus[size].outOfStock || false;
            }

            // Calculate final price with delay to ensure values are set
            setTimeout(() => {
                if (typeof calculateEditFinalPrice === 'function') {
                    calculateEditFinalPrice(size);
                }
            }, 100);
        } else {
            console.warn(`Size checkbox not found for size: ${size}`);
        }
    });
};

// Load weight-based pricing data for edit modal
window.loadEditWeightData = function (weightPricing = {}, variantStatus = {}) {
    // Clear existing weight inputs
    const mlContainer = document.getElementById('editMlContainer');
    const gContainer = document.getElementById('editGContainer');
    const kgContainer = document.getElementById('editKgContainer');
    if (mlContainer) mlContainer.innerHTML = '';
    if (gContainer) gContainer.innerHTML = '';
    if (kgContainer) kgContainer.innerHTML = '';

    const populateStatus = (group, quantity, unit) => {
        // Handle cases where quantity might already contain the unit or not
        let qty = String(quantity).trim();
        let key = qty;

        // If the key doesn't match and doesn't end with unit, try appending it
        if (!variantStatus[key] && !qty.toLowerCase().endsWith(unit.toLowerCase())) {
            key = qty + unit;
        }

        if (variantStatus[key]) {
            const comingSoonCb = group.querySelector('.edit-inline-comingsoon');
            const outOfStockCb = group.querySelector('.edit-inline-outofstock');
            if (comingSoonCb) comingSoonCb.checked = variantStatus[key].comingSoon || false;
            if (outOfStockCb) outOfStockCb.checked = variantStatus[key].outOfStock || false;
        }
    };
    ...
    if (weightPricing.ml && weightPricing.ml.length > 0) {
        weightPricing.ml.forEach(option => {
            ...
            const imageInput = lastGroup.querySelector('.edit-weight-image');
            if (imageInput) {
                imageInput.value = option.imageUrl || '';
            }
            populateStatus(lastGroup, option.quantity, 'ml');
    ...
    if (weightPricing.g && weightPricing.g.length > 0) {
        weightPricing.g.forEach(option => {
            ...
            populateStatus(lastGroup, option.quantity, 'g');
            calculateEditWeightFinalPrice(lastGroup.querySelector('.edit-weight-price'));
        });
    }
    ...
    if (weightPricing.kg && weightPricing.kg.length > 0) {
        weightPricing.kg.forEach(option => {
            ...
            populateStatus(lastGroup, option.quantity, 'kg');
            calculateEditWeightFinalPrice(lastGroup.querySelector('.edit-weight-price'));
        });
    }
};
    ...
    if (checkbox.checked && !existingInput) {
        ...
        pricingDiv.innerHTML = `
            ...
            <div>
                ...
                <div id="editSizeFinalPrice${size}" style="width: 100%; padding: 6px; border: 1px solid #27ae60; border-radius: 4px; background: #e8f5e8; color: #27ae60; font-weight: 600; text-align: center; font-size: 10px;">₹0</div>
            </div>
        `;
        ...
        const calculateSizePrice = () => {
            ...
            document.getElementById(`editSizeFinalPrice${size}`).textContent = formatCurrency(finalPrice);
        };
...
window.addEditWeightInput = function (containerId, unit) {
    ...
    newGroup.style.cssText = 'display: flex; gap: 8px; margin-bottom: 8px; align-items: center; flex-wrap: wrap;';
    ...
    newGroup.innerHTML = `
        ...
        <input type="url" class="edit-weight-image" placeholder="Image URL (optional)" style="flex: 1; min-width: 150px; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">
        <input type="text" class="edit-weight-title" placeholder="Display Title (optional)" style="flex: 1; min-width: 150px; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">
        ...
        <label style="display: flex; flex-direction: column; align-items: center; gap: 2px; font-size: 9px; font-weight: 600; cursor: pointer; color: #f59e0b;"><input type="checkbox" class="edit-inline-comingsoon">Coming</label>
        <label style="display: flex; flex-direction: column; align-items: center; gap: 2px; font-size: 9px; font-weight: 600; cursor: pointer; color: #e74c3c;"><input type="checkbox" class="edit-inline-outofstock">OOS</label>
...
window.updateEditedProduct = async function () {
    ...
    const productData = {
        ...
        comingSoon: !!document.getElementById('editProductComingSoon')?.checked,
        outOfStock: !!document.getElementById('editProductOutOfStock')?.checked
    };
    ...
    // Shared variantStatus object populated by sizes and weights
    const variantStatus = {};
    ...
    selectedSizes.forEach(size => {
        ...
        if (comingSoonCb || outOfStockCb) {
            variantStatus[size] = {
                comingSoon: comingSoonCb ? comingSoonCb.checked : false,
                outOfStock: outOfStockCb ? outOfStockCb.checked : false
            };
        }
    });
    ...
    const collectEditWeightPricing = () => {
        ...
        const processGroup = (group, unit) => {
            ...
            const imageInput = group.querySelector('.edit-weight-image');
            const imageUrl = imageInput && imageInput.value ? convertGoogleDriveUrl(imageInput.value.trim()) : '';
            const titleInput = group.querySelector('.edit-weight-title');
            const title = titleInput ? titleInput.value.trim() : '';
            ...
            if (quantity && price) {
                ...
                const key = String(quantity).trim() + unit;
                variantStatus[key] = {
                    comingSoon: comingSoonCb ? comingSoonCb.checked : false,
                    outOfStock: outOfStockCb ? outOfStockCb.checked : false
                };
    ...
    // Attach to productData
    if (Object.keys(variantStatus).length > 0) {
        productData.variantStatus = variantStatus;
    } else {
        productData.variantStatus = null;
    }

    // Collect custom pricing for edit form
    const editCustomContainer = document.getElementById('editCustomPricingContainer');
    if (editCustomContainer) {
        const rows = editCustomContainer.querySelectorAll('.edit-custom-pricing-row');
        const customPricing = [];
        rows.forEach(row => {
            const nameInput = row.querySelector('.edit-custom-name');
            const priceInput = row.querySelector('.edit-custom-price');
        ...
        productData.customPricing = customPricing;
    }

    // Collect color variants
    productData.colors = collectColorVariants(true);
    ...
    productInfo.innerHTML = `
        <div style="display: flex; align-items: center; gap: 15px;">
            ...
            <div>
                ...
                <div style="color: #059669; font-weight: 600;">${formatCurrency(product.price)}</div>
...
window.openProductModal = function (productId) {
    ...
    if (allImages.length > 1 || hasColors) {
        // Show carousel with thumbnails if multiple images OR has color variants
        let slidesHTML = '';
        let thumbnailsHTML = '';
        ...
        allImages.forEach((imgUrl, index) => {
            ...
            slidesHTML += `
                <div class="carousel-slide" data-index="${index}" ${isPrimary ? 'data-primary="true"' : ''}>
                    <img ${isPrimary ? `id="productPrimaryImage-${product.id}" data-default-src="${product.imageUrl}"` : ''} src="${imgUrl}" alt="${product.name}" onerror="this.src='data:image/svg+xml;base64,'">
                </div>
            `;

            thumbnailsHTML += `
                <div class="thumbnail ${index === 0 ? 'active' : ''}" onclick="goToSlide('${product.id}', ${index})">
                    <img src="${imgUrl}" alt="${variantName}" onerror="this.src='data:image/svg+xml;base64,'">
                </div>
            `;
        });

        carouselHTML = `
            <div class="image-carousel-container" style="position: relative;">
                <button onclick="shareProduct('${product.id}')" 
                        style="position: absolute; top: 10px; right: 10px; background: transparent; border: none; width: 40px; height: 40px; cursor: pointer; display: flex; align-items: center; justify-content: center; z-index: 10;"
                        onmouseover="this.style.opacity='0.7'"
                        onmouseout="this.style.opacity='1'">
                    <img src="share.png" alt="Share" style="width: 30px; height: 30px; object-fit: contain;">
                </button>
        ...
        // Single image - show normally with share button
        carouselHTML = `
            <div style="position: relative;">
                <img id="productPrimaryImage-${product.id}" data-default-src="${product.imageUrl}" src="${product.imageUrl}" alt="${product.name}" class="product-modal-image" onerror="this.src='data:image/svg+xml;base64,'">
                <button onclick="shareProduct('${product.id}')" 
                        style="position: absolute; top: 10px; right: 10px; background: transparent; border: none; width: 40px; height: 40px; cursor: pointer; display: flex; align-items: center; justify-content: center; z-index: 10;"
                        onmouseover="this.style.opacity='0.7'"
                        onmouseout="this.style.opacity='1'">
    ...
    modalBody.innerHTML = `
        ${carouselHTML}
        <h3 class="product-modal-name" id="productTitle-${product.id}" data-default-title="${product.name}">${product.name}</h3>
        ...
        <div class="product-modal-price-container" id="price-${product.id}">
            ${product.originalPrice && product.originalPrice > product.price && product.discount > 0 ? `
                ...
                <div class="price-row" style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
                    <span class="current-price" style="color: #27ae60; font-size: 24px; font-weight: 700;">${formatCurrency(product.price)}</span>
                    <span class="original-price" style="color: #999; font-size: 18px; text-decoration: line-through;">${formatCurrency(product.originalPrice)}</span>
            ...
            ` : `
                <div class="product-modal-price" style="color: #27ae60; font-size: 24px; font-weight: 700;">${formatCurrency(product.price)}</div>
            `}
        </div>
        ...
        ${product.deliveryCharge ? `<p style="color: #27ae60; font-weight: 600;">Delivery Charge: ${formatCurrency(product.deliveryCharge)}</p>` : ''}
        
        <!-- Color Selection Section -->
        ${product.colors && product.colors.length > 0 ? `
        <div class="color-selection" style="margin-bottom: 20px;">
            <label style="display: block; margin-bottom: 10px; font-weight: 700; color: #2c3e50;">Select Color:</label>
            <div class="color-options" style="display: flex; flex-wrap: wrap; gap: 10px;">
                <button class="color-btn" 
                        data-color-name="Default"
                        onclick="selectColor('${product.id}', -1)"
                        style="padding: 8px 15px; border: 2px solid #ddd; border-radius: 20px; background: white; cursor: pointer; font-size: 13px; font-weight: 600; transition: all 0.2s ease;">
                    Default
                </button>
            ...
            <div id="total-price-display-${product.id}">
                ${product.originalPrice && product.originalPrice > product.price && product.discount > 0 ? `
                    ...
                    <div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
                        <span style="color: #27ae60; font-size: 20px; font-weight: 700;" id="total-price-${product.id}">${formatCurrency(product.price)}</span>
                        <span style="color: #999; font-size: 16px; text-decoration: line-through;" id="total-original-${product.id}">${formatCurrency(product.originalPrice)}</span>
                ...
                ` : `
                    <div style="font-size: 20px; font-weight: 700; color: #27ae60;" id="total-price-${product.id}">${formatCurrency(product.price)}</div>
                `}
            </div>
        </div>
        
        <div class="product-modal-buttons" id="modal-buttons-container-${product.id}">
        </div>
    ...
    if (product.colors && product.colors.length > 0) {
        selectColor(product.id, -1);
    }
};

window.selectColor = function (productId, colorIndex) {
    const product = productsData.find(p => p.id === productId);
    if (!product) return;
    ...
    const thumbnailStrip = document.getElementById(`thumbnailStrip-${productId}`);
    const titleEl = document.getElementById(`productTitle-${productId}`);
    const defaultTitle = titleEl ? titleEl.getAttribute('data-default-title') : product.name;
    ...
    if (colorIndex === -1) {
        ...
        if (titleEl) titleEl.textContent = `${defaultTitle} (${colorName})`;
    }

    // Update buttons styling
    const buttons = document.querySelectorAll(`#productModal .color-btn`);
    buttons.forEach((btn) => {
        const btnColorName = btn.getAttribute('data-color-name');
        const isSelected = (colorIndex === -1 && btnColorName === "Default") ||
            (colorIndex !== -1 && btnColorName === colorName);
    ...
    // Update carousel
    if (carouselImages && thumbnailStrip && images.length > 0) {
        let slidesHTML = '';
        let thumbnailsHTML = '';
        ...
        images.forEach((imgUrl, index) => {
            ...
            slidesHTML += `
                <div class="carousel-slide" data-index="${index}" ${isFirst ? 'data-primary="true"' : ''}>
                    <img ${isFirst ? `id="productPrimaryImage-${productId}" data-default-src="${product.imageUrl}"` : ''} 
                         src="${imgUrl}" 
                         alt="${product.name} - ${colorName || 'Default'}" 
                         onerror="this.src='data:image/svg+xml;base64,'">
                </div>
            `;

            thumbnailsHTML += `
                <div class="thumbnail ${index === 0 ? 'active' : ''}" onclick="goToSlide('${productId}', ${index})">
                    <img src="${imgUrl}" alt="${colorName || 'Default'}" onerror="this.src='data:image/svg+xml;base64,'">
                </div>
            `;
        });

        carouselImages.innerHTML = slidesHTML;
        thumbnailStrip.innerHTML = thumbnailsHTML;
        carouselImages.style.transform = 'translateX(0)';
    }

    // Update Add to Cart / Go to Cart buttons based on new color
...
function updateProductModalContent(productId) {
    ...
    // Prepare all images (variant image prioritized)
    const allImages = [];
    if (variantImage) {
        allImages.push(variantImage);
    }
    ...
    if (allImages.length > 1) {
        ...
        const fallbackImage = variantImage || product.imageUrl;
        carouselHTML = `<img src="${fallbackImage}" alt="${variantName}" class="product-modal-image" onerror="this.src='data:image/svg+xml;base64,'">`;
    }

    modalBody.innerHTML = `
        ${carouselHTML}
        <h3 class="product-modal-name">${variantName}</h3>
        <div class="product-rating" style="color: #fbbf24; font-size: 16px; margin: 8px 0;">
            ${'⭐'.repeat(product.rating || 0)} ${product.rating ? `(${product.rating}/5)` : 'No rating'}
        </div>
        <div class="product-modal-price-container" id="price-${product.id}">
...
window.openProductModalFromRelated = function (productId) {
    // Reuse the single source of truth for modal rendering.
    // This prevents subtle HTML/ID mismatches that break size-wise pricing.
...
window.openProductModalFromCart = function (productId) {
    ...
    const variantName = cartItem ? (cartItem.variantDisplayName || cartItem.name || product.name) : product.name;
    const variantImage = cartItem ? (cartItem.variantImageUrl || cartItem.imageUrl || product.imageUrl) : product.imageUrl;
    ...
    // Prepare images (if color variant selected, show only variant images)
    const allImages = [];
    const selectedColor = cartItem ? cartItem.selectedColor : '';

    if (selectedColor && product.colors && product.colors.length > 0) {
        const colorVariant = product.colors.find(c => c.colorName === selectedColor);
    ...
    // Fallback to default images only if no variant images found
    if (allImages.length === 0) {
        if (variantImage) allImages.push(variantImage);
        if (product.imageUrl && !allImages.includes(product.imageUrl)) allImages.push(product.imageUrl);
        if (product.subImages && product.subImages.length > 0) {
            product.subImages.forEach(img => {
                if (img && !allImages.includes(img)) allImages.push(img);
            });
    ...
    modalBody.innerHTML = `
        ...
        <div style="background: #f8f9fa; padding: 15px; border-radius: 8px; margin: 15px 0; border-left: 4px solid #27ae60;">
            ...
            ${selectedColor ? `<p style="margin: 5px 0;"><strong>Selected Color:</strong> ${selectedColor}</p>` : ''}
            ...
            ${originalPrice > currentPrice && discount > 0 ? `
                <div style="margin: 10px 0; padding: 12px; background: white; border-radius: 6px; border: 1px solid #e0e0e0;">
                    ...
                    <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap; margin-bottom: 8px;">
                        <span style="color: #27ae60; font-size: 18px; font-weight: 700;">${formatCurrency(currentPrice)}</span>
                        <span style="color: #999; font-size: 16px; text-decoration: line-through;">${formatCurrency(originalPrice)}</span>
                ...
                <div style="margin: 10px 0; font-size: 14px;">
                    <div style="display: flex; justify-content: space-between; margin: 5px 0;">
                        <span>Subtotal (${quantity} × ${formatCurrency(currentPrice)}):</span>
                    ...
                    <div style="display: flex; justify-content: space-between; margin: 5px 0; color: #999; font-size: 10px;">
                        <span>Original subtotal (${quantity} × ${formatCurrency(originalPrice)}):</span>
                        <span style="text-decoration: line-through;">${formatCurrency(originalSubtotal)}</span>
                    </div>
            ...
            ` : `
                <div style="margin: 10px 0; padding: 12px; background: white; border-radius: 6px; border: 1px solid #e0e0e0;">
                    <div style="display: flex; justify-content: space-between; margin: 5px 0;">
                        ...
                        <span style="font-weight: 600; color: #27ae60;">${formatCurrency(currentPrice)}</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; margin: 5px 0;">
            ...
            ${deliveryCharge > 0 ? `
                <div style="display: flex; justify-content: space-between; margin: 8px 0; padding: 8px 0; border-top: 1px solid #e0e0e0;">
                    ...
                    <span style="color: #27ae60; font-weight: 600;">${formatCurrency(deliveryCharge)}</span>
                </div>
            ` : `
                <div style="display: flex; justify-content: space-between; margin: 8px 0; padding: 8px 0; border-top: 1px solid #e0e0e0;">
        ...
        <div class="product-modal-buttons">
            <button class="btn-order" onclick="handleOrderNowFromCart('${product.id}', '${size}', ${quantity}, '${selectedColor}')" style="background: #fbbf24; color: #000; font-weight: 600;">Order Now</button>
        </div>
...
window.loadRelatedProducts = function (currentProductId, currentCategory) {
    ...
    const generateProductCard = (product) => {
        const hasSizes = product.sizes && product.sizes.length > 0;
        const sizeBadges = hasSizes
            ? product.sizes.map(size => `<span class="size-badge" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 1px 4px; border-radius: 8px; font-size: 8px; font-weight: 600; display: inline-block;">${size}</span>`).join('')
            : '';
        const sizeSection = `<div class="product-sizes" style="margin-bottom: 4px; display: flex; align-items: center; gap: 2px; flex-wrap: wrap;"><img src="mark.png" alt="Size" style="width: 14px; height: 14px; object-fit: contain;">${sizeBadges}</div>`;
        ...
        <div class="related-product-card" data-product-id='${product.id}' onclick="openProductModalFromRelated('${product.id}')">
            ...
            ${(product.originalPrice && product.originalPrice > product.price && product.discount > 0) ? `
                <div style="display: flex; align-items: center; gap: 6px; flex-wrap: wrap;">
                    <span class="related-product-original-price">${formatCurrency(product.originalPrice)}</span>
                    <span class="related-product-price">${formatCurrency(product.price)}</span>
                    <span class="related-product-discount">${product.discount}% off</span>
                </div>
            ` : `
                <div class="related-product-price">${formatCurrency(product.price)}</div>
            `}
        </div>
    `;
    };
...
window.loadRelatedProducts3 = function (currentProductId, currentCategory) {
    ...
    const generateProductCard = (product) => {
        ...
        <div class="related-product-card" data-product-id='${product.id}' onclick="openProductModalFromRelated('${product.id}')">
            ...
            ${(product.originalPrice && product.originalPrice > product.price && product.discount > 0) ? `
                <div style="display: flex; alignments: center; gap: 6px; flex-wrap: wrap;">
                    <span class="related-product-original-price">${formatCurrency(product.originalPrice)}</span>
                    <span class="related-product-price">${formatCurrency(product.price)}</span>
                    <span class="related-product-discount">${product.discount}% off</span>
                </div>
...
window.openConfirmOrderModal = async function (productId) {
    ...
    const sizeInputForVariant = document.getElementById(`selected-size-${productId}`);
    ...
    if (window.orderFromCart && window.orderFromCart.productId === productId) {
        ...
        if (selectedSize && product.sizePricing && product.sizePricing[selectedSize]) {
            ...
            const sizeDiscount = (product.sizeDiscounts && product.sizeDiscounts[selectedSize] !== undefined) ? product.sizeDiscounts[selectedSize] : discount;
            currentPrice = sizeDiscount > 0 ? Math.round((originalPrice - (originalPrice * sizeDiscount / 100)) * 100) / 100 : originalPrice;
        ...
        // Check if it's a weight-based option
            ...
            if (weightOption) {
                ...
                discount = weightOption.discount || 0;
                if (weightOption.finalPrice !== undefined) {
                    currentPrice = weightOption.finalPrice;
                } else {
                    currentPrice = discount > 0 ? Math.round((originalPrice - (originalPrice * discount / 100)) * 100) / 100 : originalPrice;
                }
            }
        }
        // Check if it's a custom pricing option
        else if (selectedSize && product.customPricing && product.customPricing.length > 0) {
            const customOption = product.customPricing.find(opt => opt.name === selectedSize);
            if (customOption) {
                originalPrice = customOption.originalPrice != null ? customOption.originalPrice : (customOption.price || originalPrice);
                discount = customOption.discount || discount;
                currentPrice = discount > 0 ? Math.round((originalPrice - (originalPrice * discount / 100)) * 100) / 100 : originalPrice;
            }
        }
        else {
            if (discount > 0) {
                currentPrice = Math.round((originalPrice - (originalPrice * discount / 100)) * 100) / 100;
            }
        }
    }
    let variantName = product.name;
    let variantImage = product.imageUrl;
    if (window.orderFromCart && window.orderFromCart.productId === productId) {
        variantName = window.orderFromCart.variantName || product.name;
        variantImage = window.orderFromCart.variantImage || product.imageUrl;
    } else {
        const variantOverrides = getVariantOverridesFromInput(sizeInputForVariant);
        const variantDetails = getVariantDetails(product, selectedSize, variantOverrides);
        variantName = variantDetails.displayName || product.name;
        variantImage = variantDetails.imageUrl || product.imageUrl;
    }
    currentOrderVariant = { productId, name: variantName, image: variantImage };
    ...
    modalBody.innerHTML = `
        <div style="margin-bottom: 20px; padding: 15px; background: #f8f9fa; border-radius: 10px;">
            ...
            <div style="display: flex; gap: 15px; align-items: center;">
                <img src="${variantImage}" alt="${variantName}" style="width: 60px; height: 60px; object-fit: cover; border-radius: 8px;">
                <div>
                    <div style="font-weight: 600; color: #2c3e50;">${variantName}</div>
                    ${selectedSize ? `<div style="color: #666; font-size: 10px;">Size: ${selectedSize}</div>` : ''}
                    ${(() => {
            const colorInput = document.getElementById(`selected-color-${productId}`);
            const color = colorInput ? colorInput.value : '';
            return color ? `<div style="color: #666; font-size: 10px;">Color: ${color}</div>` : '';
        })()}
                    ${discount && discount > 0 && originalPrice > currentPrice ? `
                        <div style="margin: 5px 0;">
                            ...
                            <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
                                <span style="color: #27ae60; font-weight: 600;">${formatCurrency(currentPrice)}</span>
                                <span style="color: #999; font-size: 13px; text-decoration: line-through;">${formatCurrency(originalPrice)}</span>
                        ...
                        <div style="color: #27ae60; font-weight: 600; margin-top: 3px;">${formatCurrency(currentPrice)} x ${quantity} = ${formatCurrency(subtotal)}</div>
                    ` : `
                        <div style="color: #27ae60; font-weight: 600;">${formatCurrency(currentPrice)} x ${quantity} = ${formatCurrency(subtotal)}</div>
                    `}
                    ${deliveryCharge > 0 ? `<div style="color: #666; font-size: 10px;">Delivery: ${formatCurrency(deliveryCharge)}</div>` : ''}
                    ${discount && discount > 0 && originalPrice > currentPrice ? `
                        <div style="margin-top: 5px; padding: 5px; background: #e8f5e8; border-radius: 4px;">
                            <div style="font-size: 11px; color: #27ae60; font-weight: 600;">💰 You Save: ${formatCurrency((originalPrice - currentPrice) * quantity)}</div>
                        </div>
                    ` : ''}
                    <div style="font-weight: 700; color: #e74c3c; font-size: 16px; margin-top: 5px;" id="finalTotalAmount">Total: ${formatCurrency(totalAmount)}</div>
                </div>
            </div>
        </div>
        ...
        return `
            <div style="display: flex; gap: 15px; align-items: center; margin-bottom: 15px; padding: 10px; background: white; border-radius: 8px; border: 1px solid #e0e0e0;">
                ...
                <div style="flex: 1;">
                    ...
                    ${item.selectedColor ? `<div style="color: #666; font-size: 11px;">Color: ${item.selectedColor}</div>` : ''}
                    <div style="color: #27ae60; font-weight: 600; font-size: 10px;">${formatCurrency(item.price)} x ${item.quantity} = ${formatCurrency(item.price * item.quantity)}</div>
                    ${deliveryCharge > 0 ? `<div style="color: #666; font-size: 11px;">Delivery: ${formatCurrency(deliveryCharge)}</div>` : ''}
                </div>
                <div style="font-weight: 700; color: #e74c3c; font-size: 14px;">${formatCurrency(itemTotal)}</div>
            </div>
        `;
    }).join('');
...
window.confirmOrderForAllCartItems = async function (totalAmount) {
    ...
    try {
        ...
        // Step 3: Create a SINGLE combined order with items array
        const itemsArray = cartItems.map((item, index) => {
            ...
            return {
                ...
                originalPrice: product ? (product.originalPrice || item.price) : item.price,
        ...
        const combinedOrderData = {
            ...
            walletUsed: appliedWalletAmount,
            ...
            status: 'Pending',
            orderType: 'Combined Cart Order',
            itemCount: cartItems.length
        };

        console.log(`Combined Order Created with ${itemsArray.length} items: Total ₹${finalTotalAmount}`);

        // Handle payment method
        if (selectedPaymentMethod === 'online') {
            // For online payment, store order data globally and open Razorpay modal
            window.allCartOrdersData = combinedOrderData;
            ...
            // Cash on Delivery - Save single combined order to Firebase
            const orderId = await saveOrderToFirebase(combinedOrderData, true); // Pass flag to skip individual cashback

            if (orderId) {
                // Calculate 1% combined cashback (based on original amount before wallet deduction)
                const combinedCashbackPerHundred = 1; // 1%
                const combinedCashbackAmount = Math.round((totalAmount * combinedCashbackPerHundred) / 100 * 100) / 100;

                // Add cashback to the order - PENDING (will be earned after order completion)
                if (combinedCashbackAmount > 0) {
                    const orderRef = doc(db, 'orders', orderId);
                    await setDoc(orderRef, {
                        cashbackAmount: combinedCashbackAmount,
                        cashbackStatus: 'pending'
                    }, { merge: true });

                    // Add combined cashback to PENDING wallet (will be earned after order completion)
                    const userRef = doc(db, 'users', currentUser.uid);
                    const userDoc = await getDoc(userRef);
                    const currentPending = userDoc.exists() ? (userDoc.data().pendingCashback || 0) : 0;
                    ...
                    console.log(`Pending cart cashback added: ₹${currentPending} → ₹${currentPending + combinedCashbackAmount}`);
                    ...
                    const walletMessage = appliedWalletAmount > 0 ? `<br>💳 Wallet used: ${formatCurrency(appliedWalletAmount)}<br>💰 Final amount: ${formatCurrency(finalAmountPaid)}` : '';

                    showNotification(`🎉 Order placed successfully!<br>${cartItems.length} items in single order<br>Total amount: ${formatCurrency(totalAmount)}${walletMessage}<br>💰 Cashback: ${formatCurrency(combinedCashbackAmount)} added to pending wallet!<br>Payment: Cash on Delivery`, 'success', 8000);
                } else {
                    const finalAmountPaid = Math.round((totalAmount - appliedWalletAmount) * 100) / 100;
                    const walletMessage = appliedWalletAmount > 0 ? `<br>💳 Wallet used: ${formatCurrency(appliedWalletAmount)}<br>💰 Final amount: ${formatCurrency(finalAmountPaid)}` : '';

                    showNotification(`🎉 Order placed successfully!<br>${cartItems.length} items in single order<br>Total amount: ${formatCurrency(totalAmount)}${walletMessage}<br>Payment: Cash on Delivery`, 'success', 6000);
                }
                ...
                showNotification('❌ Order failed to process. Please try again.', 'error');
            }
        }
    } catch (error) {
...
window.openConfirmOrderModalFromCart = async function (productId) {
    ...
    const subtotal = Math.round((currentPrice * quantity) * 100) / 100;
    const totalAmount = Math.round((subtotal + deliveryCharge) * 100) / 100;
    const variantName = cartItem.variantDisplayName || cartItem.name || product.name;
    const variantImage = cartItem.variantImageUrl || cartItem.imageUrl || product.imageUrl;
    currentOrderVariant = { productId, name: variantName, image: variantImage };
        ...
        <div style="margin-bottom: 20px; padding: 15px; background: #f8f9fa; border-radius: 10px;">
            ...
            <div style="display: flex; gap: 15px; align-items: center;">
                ...
                <div>
                    <div style="font-weight: 600; color: #2c3e50;">${variantName}</div>
                    ${selectedSize ? `<div style="color: #666; font-size: 10px;">Size: ${selectedSize}</div>` : ''}
                    ${cartItem.selectedColor ? `<div style="color: #666; font-size: 10px;">Color: ${cartItem.selectedColor}</div>` : ''}
                    ${discount && discount > 0 && originalPrice > currentPrice ? `
                        <div style="margin: 5px 0;">
                            <div style="font-size: 11px; color: #999; margin-bottom: 2px;">Original Price</div>
                            <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
...
window.confirmOrderWithAddress = async function (productId, quantity, selectedSize, currentPrice, deliveryCharge, totalAmount) {
    ...
    // Create order data
    const variantName = (currentOrderVariant && currentOrderVariant.productId === productId) ? (currentOrderVariant.name || product.name) : product.name;
    const variantImage = (currentOrderVariant && currentOrderVariant.productId === productId) ? (currentOrderVariant.image || product.imageUrl) : product.imageUrl;
    ...
    if (selectedPaymentMethod === 'online') {
        ...
        if (orderId) {
            ...
            if (window.orderFromCart && window.orderFromCart.productId === productId) {
                ...
                showNotification(`Order placed successfully! 🎉<br>Order ID: ${orderId}<br>Total: ${formatCurrency(totalAmount)}<br>Payment: Cash on Delivery<br>Item removed from cart.`, 'success', 6000);
                // Clear the cart flag
                window.orderFromCart = null;
            } else {
                showNotification(`Order placed successfully! 🎉<br>Order ID: ${orderId}<br>Total: ${formatCurrency(totalAmount)}<br>Payment: Cash on Delivery`, 'success', 6000);
            }
            closeConfirmOrderModal();
            closeProductModal();
        } else {
...
window.selectSize = function (productId, selectedSize) {
    ...
    const product = productsData.find(p => p.id === productId);
    if (product) {
        const sizeImage = getImageForSelectedSize(product, selectedSize);
        updateProductModalPrimaryImage(productId, sizeImage);

        const sizeTitle = getTitleForSelectedSize(product, selectedSize);
        updateProductModalTitle(productId, sizeTitle);

        if (selectedSizeInput) {
            if (sizeTitle) {
                selectedSizeInput.dataset.displayName = sizeTitle;
            } else {
                delete selectedSizeInput.dataset.displayName;
            }

            if (sizeImage) {
                selectedSizeInput.dataset.imageUrl = sizeImage;
            } else {
                delete selectedSizeInput.dataset.imageUrl;
            }
        }
...
function getImageForSelectedSize(product, selectedSize) {
    if (!product || !selectedSize) return '';

    // Weight-based option (ml/g/kg prefixed)
    if (selectedSize.includes('-') && product.weightPricing) {
        const [unit, quantity] = selectedSize.split('-');
        const options = product.weightPricing[unit];
        if (options && options.length > 0) {
            const match = options.find(option => String(option.quantity) === quantity);
...
function getVariantDetails(product, selectedSize, overrides = {}) {
    if (!product) {
        return { displayName: '', imageUrl: '' };
    }

    const titleFromHelper = getTitleForSelectedSize(product, selectedSize);
    const imageFromHelper = getImageForSelectedSize(product, selectedSize);

    const displayName = (overrides.displayName || titleFromHelper || (selectedSize ? `${product.name} - ${selectedSize}` : '') || product.name).trim();
    const imageUrl = overrides.imageUrl || imageFromHelper || product.imageUrl;
...
function getVariantOverridesFromInput(inputEl) {
    if (!inputEl) return {};
    const overrides = {};
    if (inputEl.dataset.displayName) {
        overrides.displayName = inputEl.dataset.displayName;
    }
    if (inputEl.dataset.imageUrl) {
        overrides.imageUrl = inputEl.dataset.imageUrl;
    }
    return overrides;
}

function getTitleForSelectedSize(product, selectedSize) {
    if (!product || !selectedSize) return '';
...
function updateProductModalPrimaryImage(productId, imageUrl) {
    const primaryImg = document.getElementById(`productPrimaryImage-${productId}`);
    if (!primaryImg) return;

    const defaultSrc = primaryImg.getAttribute('data-default-src') || primaryImg.src;
    if (imageUrl && imageUrl.trim()) {
        primaryImg.src = imageUrl;
    } else if (defaultSrc) {
        primaryImg.src = defaultSrc;
    }
}

// Helper: find a weight option in any unit bucket matching the quantity string
function findWeightOption(product, selectedSize) {
    if (!product.weightPricing) return null;
    for (const unit of ['ml', 'g', 'kg']) {
        const options = product.weightPricing[unit];
        if (!options) continue;
        // New format: selectedSize === item.quantity (e.g. '300g')
        let match = options.find(item => item.quantity === selectedSize);
        // Old format: selectedSize = 'ml-300g' → split and match
        if (!match && selectedSize.includes('-')) {
            const [, quantityVal] = selectedSize.split('-');
            match = options.find(item => String(item.quantity) === quantityVal);
        }
...
window.updatePriceBySize = function (productId) {
    ...
    if (selectedSize) {
        ...
        // Check if it's a weight-based option
            ...
            const weightOption = findWeightOption(product, selectedSize);
            if (weightOption) {
                originalPrice = weightOption.originalPrice || weightOption.price;
                const weightDiscount = weightOption.discount || 0;
                if (weightOption.finalPrice !== undefined) {
                    currentPrice = weightOption.finalPrice;
                } else {
                    currentPrice = weightDiscount > 0
                        ? Math.round((originalPrice - (originalPrice * weightDiscount / 100)) * 100) / 100
                        : originalPrice;
                }
    ...
    if (selectedSize) {
        if (product.sizePricing && product.sizePricing[selectedSize] && product.sizeDiscounts && product.sizeDiscounts[selectedSize]) {
            ...
            const wo = findWeightOption(product, selectedSize);
            if (wo && wo.discount) actualDiscount = wo.discount;
        }
    }
    ...
    if (originalPrice > currentPrice && actualDiscount > 0) {
        priceContainer.innerHTML = `
            <div class="special-price-label" style="color: #27ae60; font-size: 14px; font-weight: 600; margin-bottom: 5px;">Special price</div>
            <div class="price-row" style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
                <span class="current-price" style="color: #27ae60; font-size: 24px; font-weight: 700;">${formatCurrency(currentPrice)}</span>
                <span class="original-price" style="color: #999; font-size: 18px; text-decoration: line-through;">${formatCurrency(originalPrice)}</span>
        ...
        priceContainer.innerHTML = `<div class="product-modal-price" style="color: #27ae60; font-size: 24px; font-weight: 700;">${formatCurrency(currentPrice)}</div>`;
    }
...
window.updateTotalPrice = function (productId) {
    ...
    if (originalPrice > currentPrice && discount > 0) {
        totalPriceDisplayElement.innerHTML = `
            ...
            <div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
                <span style="color: #27ae60; font-size: 20px; font-weight: 700;">${formatCurrency(totalAmount)}</span>
                <span style="color: #999; font-size: 16px; text-decoration: line-through;">${formatCurrency(totalOriginalAmount)}</span>
                <span style="background: #27ae60; color: white; padding: 4px 8px; border-radius: 6px; font-size: 10px; font-weight: 600;">${discount}% off</span>
            </div>
            <div style="font-size: 11px; color: #666; margin-top: 4px;">You save: ${formatCurrency(totalOriginalAmount - totalAmount)}</div>
        `;
    } else {
        totalPriceDisplayElement.innerHTML = `
            <div style="font-size: 20px; font-weight: 700; color: #27ae60;">${formatCurrency(totalAmount)}</div>
            ${quantity > 1 ? `<div style="font-size: 11px; color: #666; margin-top: 4px;">${quantity} items × ${formatCurrency(currentPrice)} each</div>` : ''}
        `;
    }
};
...
// Function to update product modal buttons based on cart status and variant status (size-specific)
function updateProductModalButtons(productId) {
    const product = productsData.find(p => p.id === productId);
    if (!product) return;

    const buttonsContainer = document.getElementById(`modal-buttons-container-${productId}`);
    if (!buttonsContainer) return;

    // Get currently selected size
    const selectedSizeInput = document.getElementById(`selected-size-${productId}`);
    let selectedSize = selectedSizeInput ? selectedSizeInput.value : '';

    // If no size is selected yet (initial modal open), find the default size
    if (!selectedSize) {
        if (product.sizes && product.sizes.length > 0) {
            selectedSize = product.sizes[0];
        } else if (product.weightPricing) {
            if (product.weightPricing.ml && product.weightPricing.ml.length > 0) {
                selectedSize = `ml-${product.weightPricing.ml[0].quantity}`;
            } else if (product.weightPricing.g && product.weightPricing.g.length > 0) {
                selectedSize = `g-${product.weightPricing.g[0].quantity}`;
            } else if (product.weightPricing.kg && product.weightPricing.kg.length > 0) {
                selectedSize = `kg-${product.weightPricing.kg[0].quantity}`;
            }
        }
    }

    let isComingSoon = product.comingSoon || false;
    let isOutOfStock = product.outOfStock || false;

    // Convert selectedSize to variantStatus key format if it's a weight (e.g., 'ml-500' -> '500ml')
    let statusKey = selectedSize;
    if (selectedSize && selectedSize.includes('-')) {
        const parts = selectedSize.split('-');
        if (parts.length === 2) {
            statusKey = parts[1] + parts[0];
        }
    }

    // Check size-specific variants
    if (statusKey && product.variantStatus && product.variantStatus[statusKey]) {
        if (product.variantStatus[statusKey].comingSoon) isComingSoon = true;
        if (product.variantStatus[statusKey].outOfStock) isOutOfStock = true;
    }

    if (isComingSoon) {
        buttonsContainer.innerHTML = `
            <button class="btn-order" style="width: 100%; background: #111827; color: #fbbf24; font-weight: 800; cursor: not-allowed;" onclick="showNotification('Coming soon', 'warning');">Coming soon</button>
        `;
        return;
    }
    ...
    if (isOutOfStock) {
        buttonsContainer.innerHTML = `
            <button class="btn-order" style="width: 100%; background: #e74c3c; color: white; font-weight: 800; cursor: not-allowed;" onclick="showNotification('Out of stock', 'warning');">Out of Stock</button>
        `;
        return;
    }

    // Check if this specific product with this specific size AND color is in cart
    const selectedColorInput = document.getElementById(`selected-color-${productId}`);
    let selectedColor = selectedColorInput ? selectedColorInput.value : '';

    const isInCartWithVariant = cartItems.some(item =>
        item.id === productId &&
        item.selectedSize === selectedSize &&
        (item.selectedColor || '') === (selectedColor || '')
    );

    let cartBtnText = isInCartWithVariant ? 'Go to Cart' : 'Add to Cart';
    let cartBtnBg = isInCartWithVariant ? '#27ae60' : '#3498db';
    let cartBtnAction = isInCartWithVariant ? "closeProductModal(); setActiveTab('cart');" : `addToCartWithQuantity('${productId}')`;

    buttonsContainer.innerHTML = `
        <button class="btn-cart" id="cartBtn-${productId}" onclick="${cartBtnAction}" style="background: ${cartBtnBg}; color: white;">${cartBtnText}</button>
        <button class="btn-order" onclick="handleOrderNow('${productId}')">Order Now</button>
    `;
}
...
// Open appropriate variant modal (Color first, then Size)
window.openQuantityVariantModal = function (productId) {
    ...
    const hasColorVariants = checkProductHasColorVariants(product);
    if (hasColorVariants) {
        showColorVariantModal(product);
        return;
    }

    // Then check for size variants
    const hasSizeVariants = checkProductHasSizeVariants(product);
    if (hasSizeVariants) {
        showSizeVariantModal(product);
    } else {
        // Fallback or simple increase if no variants (should not normally happen with current UI)
        handleAddToCart(productId);
    }
};

window.isProductInCart = function (productId, selectedSize = '', selectedColor = '') {
...
window.increaseQuantity = async function (productId, selectedSize = '', selectedColor = '', event) {
    if (!currentUser) {
        showNotification('Please login or register first to continue.', 'warning');
        showLoginModal();
        return;
    ...
    // Find existing item with matching product ID, size, AND color
    ...
    if (existingItem) {
        ...
        console.log(`🔢 Increased quantity to: ${existingItem.quantity} for ${product.name} (${selectedSize}) Color: ${selectedColor}`);
    } else {
        // Use color specific image if available
        let currentImageUrl = product.imageUrl;
        if (selectedColor) {
            const colorVariant = product.colors ? product.colors.find(c => c.colorName === selectedColor) : null;
            if (colorVariant && colorVariant.images && colorVariant.images.length > 0) {
                currentImageUrl = colorVariant.images[0];
            }
        }
        ...
        console.log(`➕ Added new item with Color image: ${product.name} (${selectedSize}) Color: ${selectedColor}`);
    }
    ...
    try {
        ...
        const clickedButton = event && event.target ? event.target : document.querySelector(`[onclick*="increaseQuantity('${productId}', '${selectedSize}', '${selectedColor || ''}')"]`);
        if (clickedButton) {
...
window.decreaseQuantity = async function (productId, selectedSize = '', selectedColor = '') {
    if (!currentUser) return;
...
window.getAddButtonHTML = function (productId, isRelated = false, selectedSize = '') {
    if (isProductOutOfStock(productId)) {
        const buttonClass = isRelated ? 'related-product-add-overlay' : 'product-add-overlay';
        return `<button class="${buttonClass}" style="background: #9ca3af; cursor: not-allowed;" onclick="event.stopPropagation(); showNotification('Out of stock', 'warning');">OUT</button>`;
    }
    ...
    if (!selectedSize) {
        ...
        if (product) {
            if (product.sizes && product.sizes.length > 0) {
                ...
                if (product.weightPricing.ml && product.weightPricing.ml.length > 0) {
                    selectedSize = `${product.weightPricing.ml[0].quantity}`;
                } else if (product.weightPricing.g && product.weightPricing.g.length > 0) {
                    selectedSize = `${product.weightPricing.g[0].quantity}`;
                } else if (product.weightPricing.kg && product.weightPricing.kg.length > 0) {
                    selectedSize = `${product.weightPricing.kg[0].quantity}`;
                }
            }
        }
    }
    ...
    if (totalQuantity > 0) {
        // Check if product has size or color variants
        if (!productsData || productsData.length === 0) {
        ...
        if (hasSizeVariants || hasColorVariants) {
            // If product has variants, show quantity controls that open variant selection flow
            return `
            ...
            const cartItem = cartItems.find(item => item.id === productId && (item.selectedSize === selectedSize || item.selectedSize === ''));
            const quantity = cartItem ? cartItem.quantity : totalQuantity;
            return `
                <div class="quantity-controls">
                    <button class="quantity-btn" onclick="event.stopPropagation(); decreaseQuantity('${productId}', '${selectedSize}')">−</button>
...
// Handle Add to Cart - Check for size and color variants
window.handleAddToCart = function (productId) {
    ...
    if (hasSizeVariants) {
        ...
        if (product.sizes && product.sizes.length > 0) {
            ...
            if (product.weightPricing.ml && product.weightPricing.ml.length > 0) {
                defaultSize = `${product.weightPricing.ml[0].quantity}`;
            } else if (product.weightPricing.g && product.weightPricing.g.length > 0) {
                defaultSize = `${product.weightPricing.g[0].quantity}`;
            } else if (product.weightPricing.kg && product.weightPricing.kg.length > 0) {
                defaultSize = `${product.weightPricing.kg[0].quantity}`;
            }
        }
...
// Function to check if product has color variants
function checkProductHasColorVariants(product) {
...
// Color Variant Modal Logic
window.showColorVariantModal = function (product) {
    const modal = document.getElementById('colorVariantModal');
    const productTitle = document.getElementById('colorVariantProduct');
    ...
    confirmBtn.textContent = 'Select a color to continue';

    product.colors.forEach(variant => {
        const item = document.createElement('div');
        item.className = 'color-variant-item';
        item.onclick = () => selectColorOption(item, variant);

        const variantImg = (variant.images && variant.images.length > 0) ? variant.images[0] : 'placeholder.jpg';

        item.innerHTML = `
            <div class="color-swatch-ring">
                <div class="color-swatch" style="background-image: url('${variantImg}'); background-size: cover; background-position: center;"></div>
            </div>
            <div class="color-variant-name">${variant.colorName}</div>
        `;
...
window.selectColorOption = function (element, variant) {
    ...
    confirmBtn.textContent = `Confirm ${variant.colorName}`;
};

window.closeColorVariantModal = function (event) {
    if (event && event.target !== document.getElementById('colorVariantModal') && event.type === 'click' && event.target.className !== 'color-variant-close') {
        return;
    }
    const modal = document.getElementById('colorVariantModal');
    if (modal) {
...
window.addSelectedColorToCart = function () {
    const modal = document.getElementById('colorVariantModal');
    ...
    // Close color modal
    closeColorVariantModal();

    // Check if it also has size variants
    const hasSizeVariants = checkProductHasSizeVariants(product);
    if (hasSizeVariants) {
        // Now show size modal, passing the color info
        showSizeVariantModalWithColor(product, selectedColorVariant.colorName);
    } else {
        // Direct add to cart with selected color and default size
        let defaultSize = '';
        if (product.sizes && product.sizes.length > 0) {
            defaultSize = product.sizes[0];
        } else if (product.weightPricing) {
            if (product.weightPricing.ml && product.weightPricing.ml.length > 0) defaultSize = `${product.weightPricing.ml[0].quantity}`;
            else if (product.weightPricing.g && product.weightPricing.g.length > 0) defaultSize = `${product.weightPricing.g[0].quantity}`;
            else if (product.weightPricing.kg && product.weightPricing.kg.length > 0) defaultSize = `${product.weightPricing.kg[0].quantity}`;
        }
        increaseQuantity(productId, defaultSize, selectedColorVariant.colorName);
    }
};

// Modified showSizeVariantModal to support optional pre-selected color
window.showSizeVariantModalWithColor = function (product, color) {
    // Pass color to main size variant modal function
    window.showSizeVariantModal(product, color);
};
...
function getVariantControlsHTML(productId, size, variantId, price) {
    // Check if a color was pre-selected in the modal
    const modal = document.getElementById('sizeVariantModal');
    const selectedColor = modal ? modal.getAttribute('data-selected-color') : '';

    // Check if this specific variant (size + color) is already in cart
    ...
    if (cartItem && cartItem.quantity > 0) {
        ...
        return `
            <div class="size-variant-quantity-controls">
                <button class="size-variant-quantity-btn" onclick="event.stopPropagation(); decreaseVariantQuantity('${productId}', '${size}', '${selectedColor}')">
                    −
                </button>
                <span class="size-variant-quantity-display" id="variant-qty-${variantId}">${cartItem.quantity}</span>
                <button class="size-variant-quantity-btn" onclick="event.stopPropagation(); increaseVariantQuantity('${productId}', '${size}', '${selectedColor}')">
                    +
                </button>
            </div>
        `;
        ...
        return `
            <button class="size-variant-add-to-cart" onclick="event.stopPropagation(); addVariantToCart('${productId}', '${size}', ${price}, '${selectedColor}')">
                ADD
            </button>
        `;
    }
...
window.showSizeVariantModal = function (product, selectedColor = '') {
    const modal = document.getElementById('sizeVariantModal');
    ...
    if (selectedColor) {
        modal.setAttribute('data-selected-color', selectedColor);
    } else {
        modal.removeAttribute('data-selected-color'); // Reset color for a clean session
    }

    // Set title
    title.textContent = product.name;

    // Determine the image to show in variants (use color image if selected)
    let displayImage = product.imageUrl;
    if (selectedColor) {
        const colorVar = product.colors ? product.colors.find(c => c.colorName === selectedColor) : null;
        if (colorVar && colorVar.images && colorVar.images.length > 0) {
            displayImage = colorVar.images[0];
        }
    }
            ...
            return `
                <div class="size-variant-item">
                    <div class="size-variant-info">
                        <img src="${displayImage}" alt="${product.name}" class="size-variant-image" 
                    ...
                    `<span class="size-variant-original-price">${formatCurrency(variant.originalPrice)}</span>` : ''}
                            </div>
                        </div>
                    </div>
...
window.increaseVariantQuantity = async function (productId, size, color = '') {
    const cartItem = cartItems.find(item => item.id === productId && item.selectedSize === size && (item.selectedColor || '') === (color || ''));
    if (cartItem) {
        cartItem.quantity += 1;
        const variantId = `${productId}-${size}`;
        const displayElement = document.getElementById(`variant-qty-${variantId}`);
        if (displayElement) displayElement.textContent = cartItem.quantity;
        updateCartCount();
        updateCartDisplayMain();
        await saveCartToFirebase();
    }
...
window.decreaseVariantQuantity = async function (productId, size, color = '') {
    const cartItem = cartItems.find(item => item.id === productId && item.selectedSize === size && (item.selectedColor || '') === (color || ''));
    if (cartItem) {
        if (cartItem.quantity > 1) {
            cartItem.quantity -= 1;
        } else {
            const index = cartItems.findIndex(item => item.id === productId && item.selectedSize === size && (item.selectedColor || '') === (color || ''));
            ...
            if (controlsContainer) {
                const product = productsData.find(p => p.id === productId);
                const variants = generateSizeVariants(product);
                const v = variants.find(varnt => varnt.size === size);
                controlsContainer.innerHTML = `
                    <button class="size-variant-add-to-cart" onclick="event.stopPropagation(); addVariantToCart('${productId}', '${size}', ${v ? v.price : 0}, '${color}')">
                        ADD
                    </button>
                `;
            }
        ...
        const variantId = `${productId}-${size}`;
        const displayElement = document.getElementById(`variant-qty-${variantId}`);
        if (displayElement) displayElement.textContent = cartItem.quantity > 0 ? cartItem.quantity : '0';
        updateCartCount();
        updateCartDisplayMain();
        await saveCartToFirebase();
    }
...
function generateSizeVariants(product) {
    ...
    if (product.sizePricing) {
        Object.entries(product.sizePricing).forEach(([size, originalPrice]) => {
            ...
            // Check for price addition in size name like "XL +50"
            const priceMatch = size.match(/\+(\d+)/);
            if (priceMatch) {
                const extraPrice = parseInt(priceMatch[1]);
                actualPrice += extraPrice;
            }

            const discount = product.discount || 0;
            const price = discount > 0 ?
                Math.round((actualPrice - (actualPrice * discount / 100)) * 100) / 100 :
                actualPrice;
        ...
        ['ml', 'g', 'kg'].forEach(unit => {
            if (product.weightPricing[unit] && product.weightPricing[unit].length > 0) {
                product.weightPricing[unit].forEach(weightOption => {
                    const originalPrice = weightOption.originalPrice || weightOption.price;
                    // Use pre-stored finalPrice if available; otherwise compute from original + own discount
                    let finalPrice;
                    if (weightOption.finalPrice !== undefined) {
                        finalPrice = weightOption.finalPrice;
                    } else {
                        const discount = weightOption.discount || 0;
                    ...
                    variants.push({
                        size: `${weightOption.quantity}`,
                        price: finalPrice,         // actual price to pay
                        originalPrice: originalPrice // strike-through price
                    });
                });
            }
        });
...
window.addVariantToCart = function (productId, size, price, color = '') {
    console.log(`Adding ${size} (Color: ${color}) of product ${productId} to cart`);

    // Add to cart with quantity 1
    addToCartWithQuantity(productId, size, 1, null, color);
    ...
    if (controlsContainer) {
        ...
        controlsContainer.innerHTML = `
            <div class="size-variant-quantity-controls">
                <button class="size-variant-quantity-btn" onclick="event.stopPropagation(); decreaseVariantQuantity('${productId}', '${size}', '${color}')">
                    −
                </button>
                <span class="size-variant-quantity-display" id="variant-qty-${variantId}">1</span>
                <button class="size-variant-quantity-btn" onclick="event.stopPropagation(); increaseVariantQuantity('${productId}', '${size}', '${color}')">
                    +
                </button>
            </div>
        `;
...
window.addToCartWithQuantity = async function (productId, selectedSize = '', quantity = 1, event = null, selectedColor = '') {
    if (isProductOutOfStock(productId)) {
        showNotification('Out of stock', 'warning');
        return;
    }
    ...
    const selectedSizeInput = document.getElementById(`selected-size-${productId}`);
    if (selectedSizeInput && !selectedSize) {
        selectedSize = selectedSizeInput.value;
    }
    ...
    if (!selectedColor) {
        const selectedColorInput = document.getElementById(`selected-color-${productId}`);
        selectedColor = selectedColorInput ? selectedColorInput.value : '';
    }
    ...
    if (product) {
        ...
        if (selectedSize) {
            ...
            // Check if it's a weight-based option (any format: "300g", "ml-100", etc.)
            else {
                const weightOption = findWeightOption(product, selectedSize);
                if (weightOption) {
                    originalPrice = weightOption.originalPrice || weightOption.price;
                    ...
                    discount = weightDiscount; // use weight-specific discount for cart item
                }
            }
        } else {
            // No size selected, use base price with discount
        ...
        if (selectedSizeInput) {
            if (selectedSizeInput.dataset.displayName) {
                variantOverrides.displayName = selectedSizeInput.dataset.displayName;
            }
            if (selectedSizeInput.dataset.imageUrl) {
                variantOverrides.imageUrl = selectedSizeInput.dataset.imageUrl;
            }
        }
        const variantDetails = getVariantDetails(product, selectedSize, variantOverrides);
        const variantName = variantDetails.displayName || product.name;
        const variantImage = variantDetails.imageUrl || product.imageUrl;

        // Use color specific image if available
        let currentImageUrl = variantImage;
        if (selectedColor) {
            const colorVariant = product.colors ? product.colors.find(c => c.colorName === selectedColor) : null;
            if (colorVariant && colorVariant.images && colorVariant.images.length > 0) {
                currentImageUrl = colorVariant.images[0];
        ...
        const existingItem = cartItems.find(item => item.id === productId && item.selectedSize === selectedSize && item.selectedColor === selectedColor);
        if (existingItem) {
            existingItem.quantity += quantity;
            existingItem.name = variantName;
            existingItem.imageUrl = currentImageUrl;
            existingItem.variantDisplayName = variantName;
            existingItem.variantImageUrl = currentImageUrl;
            existingItem.baseName = product.name;
            existingItem.baseImageUrl = product.imageUrl;
            console.log('🛒 Updated existing cart item:', existingItem.name, 'Color:', selectedColor);
        } else {
            const cartItem = {
                id: product.id,
                name: variantName,
                ...
                selectedColor: selectedColor, // Added selected color
                category: product.category,
                variantDisplayName: variantName,
                variantImageUrl: currentImageUrl
            };
            cartItems.push(cartItem);
            console.log('🛒 Added new cart item:', cartItem.name, 'Color:', selectedColor);
        }
        ...
        showNotification(`${quantity} x ${product.name}${sizeText}${colorText} added to cart!`, 'success');
...
window.orderFromCart = async function (productId) {
    ...
    try {
        ...
        if (orderId) {
            ...
            showNotification(`Order placed successfully!<br><br>Product: ${product.name}<br>Quantity: ${cartItem.quantity}<br>Total: ${formatCurrency(orderData.totalAmount)}`, 'success');
...
window.orderNowWithQuantity = async function (productId) {
    ...
    if (product) {
        ...
        // Get selected size from modal if not already set
        if (!selectedSize) {
            const selectedSizeInput = document.getElementById(`selected-size-${productId}`);
            if (selectedSizeInput) {
                selectedSize = selectedSizeInput.value;
        ...
        let orderImage = product.imageUrl;
        if (selectedColor) {
            const colorVariant = product.colors.find(c => c.colorName === selectedColor);
            if (colorVariant && colorVariant.images && colorVariant.images.length > 0) {
                orderImage = colorVariant.images[0];
            }
        }
        ...
        const orderData = {
            productId: product.id,
            productName: selectedColor ? `${product.name} (${selectedColor})` : product.name,
            productPrice: currentPrice,
            originalPrice: originalPrice,
            discount: discount,
            productImage: orderImage,
            productSize: selectedSize,
            selectedColor: selectedColor, // Captured color
            quantity: quantity,
            deliveryCharge: deliveryCharge,
        ...
        const orderId = await saveOrderToFirebase(orderData);
        if (orderId) {
            showNotification(`Order placed successfully! 🎉<br>Order ID: ${orderId}<br>Total: ${formatCurrency(orderData.totalAmount)}`, 'success', 6000);
            closeProductModal();
        } else {
            showNotification('Error placing order. Please try again.', 'error');
        }
...
window.cancelOrder = async function (orderId) {
    ...
    if (result.success) {
        ...
        if (result.totalAmountRefunded > 0) {
            message += `<br>💳 ${formatCurrency(result.totalAmountRefunded)} added to your cashback wallet (Online payment refund)`;
        }

        // Add wallet refund info if applicable
        if (result.walletRefunded > 0) {
            message += `<br>💰 ${formatCurrency(result.walletRefunded)} refunded to your wallet`;
        }

        // Add cashback deduction info if applicable
        if (result.cashbackDeducted > 0) {
            message += `<br>📉 ${formatCurrency(result.cashbackDeducted)} earned cashback deducted`;
        }
        ...
        cartItem.innerHTML = `
            ...
            <div class="cart-item-controls">
                <div class="cart-quantity-controls">
                    <button class="quantity-btn" onclick="decreaseQuantity('${item.id}', '${item.selectedSize || ''}', '${item.selectedColor || ''}')" ${item.quantity <= 1 ? 'disabled' : ''}>-</button>
                    <span class="quantity-display">${item.quantity}</span>
                    <button class="quantity-btn" onclick="increaseQuantity('${item.id}', '${item.selectedSize || ''}', '${item.selectedColor || ''}', event)">+</button>
                </div>
                <button class="cart-item-remove" onclick="removeFromCart('${item.id}', '${item.selectedSize || ''}', '${item.selectedColor || ''}')">Remove</button>
            </div>
        `;
        cartItemsContainer.appendChild(cartItem);
    });

    cartTotalAmount.textContent = `Total: ${formatCurrency(total)}`;
        ...
        cartItem.innerHTML = `
            ...
            <div class="cart-item-controls-main">
                <div class="cart-quantity-controls-main">
                    <button class="quantity-btn-main" onclick="decreaseQuantity('${item.id}', '${item.selectedSize || ''}', '${item.selectedColor || ''}')" ${item.quantity <= 1 ? 'disabled' : ''}>-</button>
                    <span class="quantity-display-main">${item.quantity}</span>
                    <button class="quantity-btn-main" onclick="increaseQuantity('${item.id}', '${item.selectedSize || ''}', '${item.selectedColor || ''}', event)">+</button>
                </div>
                <button class="cart-item-remove-main" onclick="removeFromCart('${item.id}', '${item.selectedSize || ''}', '${item.selectedColor || ''}')">Remove</button>
            </div>
        `;
        cartItemsContainer.appendChild(cartItem);
    });
...
window.removeFromCart = async function (productId, size, color = '') {
...
window.checkout = async function () {
    ...
    const orderPromises = cartItems.map(item => {
        ...
        const orderData = {
            ...
            selectedColor: item.selectedColor || '', // Added captured color
            quantity: item.quantity,
            deliveryCharge: deliveryCharge,
    ...
    try {
        ...
        if (successfulOrders.length === cartItems.length) {
            showNotification(`Checkout successful! 🎉<br>${successfulOrders.length} orders placed<br>Total amount: ${formatCurrency(total)}`, 'success', 6000);
            cartItems = [];
            updateCartCount();
            updateCartDisplay();
            updateCartDisplayMain();
...
window.handleOrderNowFromCart = function (productId, selectedSize, quantity, selectedColor = '') {
    if (!currentUser) {
        // User not logged in, show login modal
        document.getElementById('loginModal').style.display = 'flex';
        // Hide bottom navbar when modal opens
    ...
    window.orderFromCart = {
        ...
        variantName: cartItem.variantDisplayName || cartItem.name || (typeof product !== 'undefined' ? product.name : ''),
        variantImage: cartItem.variantImageUrl || cartItem.imageUrl || (typeof product !== 'undefined' ? product.imageUrl : '')
    };
    ...
    tempContainer.innerHTML = `
        ...
        <input type="hidden" id="selected-color-${productId}" value="${selectedColor}">
...
function createProductCard(product) {
    ...
    const hasWeightOptions =
        ...
        (weightPricing.g && weightPricing.g.length > 0) ||
        (weightPricing.kg && weightPricing.kg.length > 0);
    const sizeSection = `<div class="product-sizes" style="margin-bottom: 6px; display: flex; align-items: center; gap: 8px; flex-wrap: wrap;"><img src="mark.png" alt="Variants" style="width: 20px; height: 20px; object-fit: contain;">${hasSizes ? '<span style="font-size: 12px; font-weight: 700; color: #111827; text-transform: uppercase; letter-spacing: 0.6px;">Variants</span>' : ''}${sizeBadges}</div>`;
...
function getOrderHTML(order, isRefund = false) {
    // Check if this is a combined order with items array
    const isCombinedOrder = order.items && Array.isArray(order.items) && order.items.length > 0;

    // For combined orders, show items summary
    if (isCombinedOrder) {
        const itemsSummary = order.items.map((item, index) => {
            ...
            return `
                <div style="display: flex; gap: 8px; align-items: center; padding: 8px; background: #f8f9fa; border-radius: 6px; margin-bottom: 5px; cursor: pointer; transition: all 0.3s ease;" 
                     onclick="openProductModal('${productId}')" 
                    ...
                    <img src="${productImage}" alt="${itemName}" style="width: 40px; height: 40px; object-fit: contain; border-radius: 4px; background: white; border: 1px solid #e9ecef;" onerror="this.style.display='none'">
                    <div style="flex: 1; min-width: 0;">
                        <div style="font-size: 12px; font-weight: 600; color: #2c3e50; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${itemName}</div>
                        <div style="font-size: 10px; color: #666;">${itemSize ? `Size: ${itemSize}` : ''} ${itemColor ? `| Color: ${itemColor}` : ''}</div>
                        <div style="font-size: 10px; color: #27ae60; font-weight: 600;">${formatCurrency(itemPrice)} x ${itemQty} = ${formatCurrency(itemTotal)}</div>
                    </div>
        ...
        return `
                ...
                <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 12px; border-radius: 8px; margin-bottom: 15px; color: white;">
                    <div style="display: flex; align-items: center; gap: 10px;">
                        ...
                        <div>
                            <div style="font-size: 14px; font-weight: 700;">Combined Cart Order</div>
                            <div style="font-size: 11px; opacity: 0.9;">${order.itemCount || order.items.length} items in this order</div>
                        </div>
                    </div>
                </div>
                ...
                <div style="font-size: 10px; color: #666; margin-bottom: 15px;">
                    <div>Order ID: <span style="font-family: monospace; background: #f8f9fa; padding: 1px 4px; border-radius: 3px;">${order.id.substring(0, 12)}...</span></div>
                ...
                <div style="display: flex; gap: 10px; margin-bottom: 15px;">
                    <div style="flex: 1; background: #f8f9fa; padding: 10px; border-radius: 6px;">
                        <div style="font-size: 13px; color: #666; margin-bottom: 3px;">Original Amount</div>
                        <div style="font-size: 13px; color: #999; text-decoration: line-through;">${formatCurrency(order.originalAmount)}</div>
                        ${(() => {
            ...
            return `<div style="font-size: 13px; color: ${textColor}; font-weight: 600; background: ${bgColor}; padding: 6px; border-radius: 4px; margin-top: 5px;">💰 Wallet Applied: ${formatCurrency(walletAmount)}</div>`;
        })()}
                    </div>
                    <div style="flex: 1; background: #e8f5e8; padding: 10px; border-radius: 6px; text-align: center; display: flex; flex-direction: column; justify-content: center;">
                        ...
                        <div style="font-size: 18px; font-weight: 700; color: #27ae60;">${formatCurrency(order.totalAmount)}</div>
                    </div>
                </div>
    ...
    return `
            ...
            return rating > 0 ? `<div style="color: #fbbf24; font-size: 10px; margin: 2px 0;">${'⭐'.repeat(rating)} (${rating})</div>` : '';
                ...
                <div style="display: flex; gap: 10px; margin-bottom: 15px;">
                    <div style="flex: 1; background: #f8f9fa; padding: 10px; border-radius: 6px;">
                        ...
                        <div style="font-size: 13px; color: #666; margin-bottom: 3px;">Unit Price: ${formatCurrency(order.productPrice)}</div>
                        ${order.productSize ? `<div style="font-size: 13px; color: #666; margin-bottom: 3px;">Size: <strong>${order.productSize}</strong></div>` : ''}
                        ${order.selectedColor ? `<div style="font-size: 13px; color: #666; margin-bottom: 3px;">Color: <strong>${order.selectedColor}</strong></div>` : ''}
                        ${order.deliveryCharge ? `<div style="font-size: 13px; color: #27ae60; font-weight: 600;">🚚 Delivery: ${formatCurrency(order.deliveryCharge)}</div>` : ''}
                        ${(() => {
            const walletAmount = order.walletUsed || 0;
            ...
            const bgColor = walletAmount > 0 ? 'rgba(142, 68, 173, 0.1)' : 'rgba(108, 117, 125, 0.1)';
            const textColor = walletAmount > 0 ? '#8e44ad' : '#6c757d';
            return `<div style="font-size: 13px; color: ${textColor}; font-weight: 600; background: ${bgColor}; padding: 6px; border-radius: 4px; margin: 2px 0;">💰 Wallet Applied: ${formatCurrency(walletAmount)}</div>`;
        })()}
                        ${order.originalAmount && order.originalAmount !== order.totalAmount ? `<div style="font-size: 10px; color: #999;">Original Amount: ${formatCurrency(order.originalAmount)}</div>` : ''}
                    </div>
                    <div style="flex: 1; background: #e8f5e8; padding: 10px; border-radius: 6px; text-align: center; display: flex; flex-direction: column; justify-content: center;">
                        ${(() => {
            ...
            if (hasDiscount && originalPrice > order.productPrice) {
                ...
                return `
                                    ...
                                    <div style="font-size: 10px; color: #999; text-decoration: line-through; margin-bottom: 2px;">${formatCurrency(originalTotal)}</div>
                                    <div style="font-size: 10px; color: #27ae60; margin-bottom: 2px;">Final Amount</div>
                                    <div style="font-size: 18px; font-weight: 700; color: #27ae60;">${formatCurrency(order.totalAmount)}</div>
            ...
            if (order.status === 'Completed' && !order.refundRequested) {
                ...
                if (order.completedAt) {
                    ...
                    const now = new Date();
                    const hoursDifference = (now.getTime() - completedDate.getTime()) / (1000 * 60 * 60);
                ...
                if (!isWithin24Hours) {
                    return `
                        <div style="flex: 1; text-align: center; padding: 12px; background: #f8f9fa; border-radius: 8px; color: #6c757d; font-size: 13px; font-weight: 600; border: 1px solid #dee2e6;">
                            ⏱️ Return Window Closed
                        </div>
                    `;
                }
...
window.openAdminPanel = function () {
    ...
    // Hide bottom navbar when admin panel opens
    const bottomNav = document.querySelector('.app-bottom-nav');
    if (bottomNav) {
        bottomNav.style.display = 'none';
    }

    // Immediately show overview tab and load data
    showTab('overview');
...
window.openDeliveryPanel = function () {
    ...
    // Hide bottom navbar when delivery panel opens
    const bottomNav = document.querySelector('.app-bottom-nav');
    if (bottomNav) {
        bottomNav.style.display = 'none';
    }
...
window.closeDeliveryPanel = function () {
    ...
    // Show bottom navbar when delivery panel closes
    const bottomNav = document.querySelector('.app-bottom-nav');
    if (bottomNav) {
        bottomNav.style.display = 'flex';
    }
...
window.closeAdminPanel = function () {
    ...
    // Show bottom navbar when admin panel closes
    const bottomNav = document.querySelector('.app-bottom-nav');
    if (bottomNav) {
        bottomNav.style.display = 'flex';
    }
...
window.deliveryCompleteOrder = async function (orderId) {
    try {
        ...
        if (orderData && orderData.cashbackAmount > 0 && orderData.userId) {
            ...
            // Show immediate notification to delivery person
            showNotification(`Order completed! ₹${orderData.cashbackAmount} cashback will be available to customer in 24 hours ⏰`, 'success');
        } else {
            showNotification('Order marked as Completed ✅', 'success');
        }
...
window.loadOverviewData = function() {
    updateOverviewTotalUsers();
    updateOverviewOrders(7); // Default to 7 days
    updateOverviewRevenue(7); // Default to 7 days
    updateOverviewPendingOrders(); // NEW
    updateOverviewTotalProducts(); // NEW
    loadRecentCompletedOrders(); // NEW: Load the history section
    cleanupOldOrders(); // NEW: Auto-delete orders older than 8 days
    
    // Set default values in selectors
    if (document.getElementById('orderFilter')) document.getElementById('orderFilter').value = "7";
    if (document.getElementById('revenueFilter')) document.getElementById('revenueFilter').value = "7";
};

// --- NEW Dashboard History Sections ---

window.loadRecentCompletedOrders = async function() {
    const container = document.getElementById('overviewCompletedOrders');
    ...
    try {
        ...
        const firestoreTimestamp = typeof Timestamp !== 'undefined' ? Timestamp : 
                                   (window.firebase && window.firebase.firestore ? window.firebase.firestore.Timestamp : null);

        const ordersQuery = query(
            collection(db, 'orders'),
            where('createdAt', '>=', firestoreTimestamp.fromDate(cutoff))
        );
        ...
        snapshot.forEach(doc => {
            const data = doc.data();
            // Client-side status filtering to bypass index requirements
        ...
        displayRecentCompletedOrders(orders);
    } catch (error) {
        console.error('Error loading recent completed orders:', error);
    }
};

window.displayRecentCompletedOrders = function(orders) {
    const container = document.getElementById('overviewCompletedOrders');
    if (!container) return;

    if (orders.length === 0) {
        container.innerHTML = `
            <div style="grid-column: 1 / -1; text-align: center; padding: 40px; color: #95a5a6; background: #f8f9fa; border-radius: 15px; border: 2px dashed #e9ecef;">
    ...
    orders.forEach(order => {
        const dateStr = order.createdAt && order.createdAt.toDate ? 
                        order.createdAt.toDate().toLocaleString('en-IN', { dateStyle: 'medium', timeStyle: 'short' }) : 'Unknown Date';
        ...
        card.innerHTML = `
            <div style="display: flex; gap: 12px; align-items: flex-start;">
                <img src="${order.productImage || ''}" style="width: 50px; height: 50px; border-radius: 8px; object-fit: contain; background: #f8f9fa; border: 1px solid #eee;">
                <div style="flex: 1;">
                    <h5 style="margin: 0; font-size: 14px; color: #2c3e50; font-weight: 700;">${order.productName || 'Product'}</h5>
                    <div style="font-size: 11px; color: #7f8c8d; margin-top: 2px;">${dateStr}</div>
                    <div style="font-size: 11px; color: #2c3e50; font-weight: 600; margin-top: 4px;">Amt: ₹${order.totalAmount || 0} | Qty: ${order.quantity || 1}</div>
                </div>
                <div style="text-align: right;">
                     <span style="font-size: 10px; padding: 2px 6px; border-radius: 10px; background: #e8f5e8; color: #27ae60; font-weight: 800; text-transform: uppercase;">${order.status}</span>
                </div>
            </div>
            
            <div style="display: flex; gap: 8px; margin-top: 5px;">
                <button onclick="window.showInvoice('${order.id}')" style="flex: 1; padding: 8px; background: #f0f4ff; color: #3498db; border: none; border-radius: 8px; font-size: 11px; font-weight: 700; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 5px;">
                    📄 Invoice
                </button>
                <button onclick="window.openUserLocation('${order.id}')" style="flex: 1; padding: 8px; background: #fef5e7; color: #f39c12; border: none; border-radius: 8px; font-size: 11px; font-weight: 700; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 5px;">
                    📍 Location
                </button>
            </div>
        `;
...
window.cleanupOldOrders = async function() {
    try {
        console.log('🧹 Running smart auto-cleanup for closed orders older than 8 days...');
        const now = new Date();
        ...
        // Fetch closed orders (Completed or Rejected)
        const ordersQuery = query(
            collection(db, 'orders'),
        ...
        snapshot.forEach(docSnap => {
            ...
            // Use completedAt, rejectedAt or deliveredAt for the cleanup age
            const closedDate = data.completedAt || data.rejectedAt || data.deliveredAt || data.createdAt;
            
            if (closedDate && closedDate.toDate) {
                const ageMs = now.getTime() - closedDate.toDate().getTime();
        ...
        if (deletedCount > 0) {
            console.log(`✅ Cleaned up ${deletedCount} old closed orders.`);
        }
    } catch (error) {
        console.error('Error during smart auto-cleanup:', error);
    }
};

window.updateOverviewTotalUsers = async function() {
    try {
        ...
        const countDisplay = document.getElementById('overviewTotalUsers');
        if (countDisplay) countDisplay.textContent = userCount;
        
        // Also update the count in users tab if visible
        const userCountDisplay = document.getElementById('userCountDisplay');
        if (userCountDisplay) userCountDisplay.textContent = `Total Registered Users: ${userCount}`;
    } catch (error) {
        console.error('Error fetching user count:', error);
    }
};

window.updateOverviewOrders = async function(days) {
    try {
        const ordersDisplay = document.getElementById('overviewTotalOrders');
        if (ordersDisplay) ordersDisplay.textContent = '...';
        ...
        if (!firestoreTimestamp) {
             console.error('Firestore Timestamp not found');
             return;
        }
        ...
        snapshot.forEach(doc => {
            const data = doc.data();
            // User requested: "last 7 days completed order count show hoga"
            if (data.status === 'Completed' || data.status === 'Delivered') {
                count++;
            }
        });

        if (ordersDisplay) ordersDisplay.textContent = count;
    } catch (error) {
        console.error('Error updating overview orders:', error);
        const ordersDisplay = document.getElementById('overviewTotalOrders');
        if (ordersDisplay) ordersDisplay.textContent = 'Error';
    }
};

window.updateOverviewPendingOrders = async function() {
    try {
        const pendingDisplay = document.getElementById('overviewPendingOrders');
        ...
        const snapshot = await getDocs(ordersQuery);
        if (pendingDisplay) pendingDisplay.textContent = snapshot.size;
    } catch (error) {
        console.error('Error updating pending orders:', error);
        const pendingDisplay = document.getElementById('overviewPendingOrders');
        if (pendingDisplay) pendingDisplay.textContent = 'Error';
    }
};

window.updateOverviewTotalProducts = async function() {
    try {
        const productDisplay = document.getElementById('overviewTotalProducts');
        if (productDisplay) productDisplay.textContent = '...';

        const snapshot = await getDocs(collection(db, 'products'));
        if (productDisplay) productDisplay.textContent = snapshot.size;
    } catch (error) {
        console.error('Error updating total products:', error);
        const productDisplay = document.getElementById('overviewTotalProducts');
        if (productDisplay) productDisplay.textContent = 'Error';
    }
};

window.updateOverviewRevenue = async function(days) {
    try {
        const revenueDisplay = document.getElementById('overviewTotalRevenue');
        if (revenueDisplay) revenueDisplay.textContent = '...';
        ...
        snapshot.forEach(doc => {
            ...
            // Ensure we use a clean numeric value for amount
        ...
        console.error('Error updating overview revenue:', error);
        const revenueDisplay = document.getElementById('overviewTotalRevenue');
        if (revenueDisplay) revenueDisplay.textContent = 'Error';
    }
};
            ...
            refundElement.innerHTML = `
                ...
                <div style="display: flex; gap: 10px; margin-bottom: 15px;">
                    ...
                    <div style="flex: 1; background: #ffe8e8; padding: 10px; border-radius: 6px; text-align: center; display: flex; flex-direction: column; justify-content: center; border: 2px solid #dc3545;">
                        ...
                        <div style="font-size: 18px; font-weight: 700; color: #dc3545;">${formatCurrency(order.totalAmount)}</div>
...
window.completeRefund = async function (orderId) {
    try {
        ...
        // Extract cashback that was awarded for this order and needs to be deducted
        const cashbackToDeduct = orderData.cashbackAmount || 0;

        // Add refund amount to user's earned cashback wallet, but subtract the previously awarded cashback
        if (userId && totalAmount > 0) {
            const userRef = doc(db, 'users', userId);
            const userDoc = await getDoc(userRef);
            const currentEarned = userDoc.exists() ? (userDoc.data().earnedCashback || 0) : 0;

            let newEarned = currentEarned + totalAmount;
            if (cashbackToDeduct > 0) {
                newEarned -= cashbackToDeduct;
                console.log(`Deducting ₹${cashbackToDeduct} previously earned cashback from the wallet refund`);
            }
            ...
            console.log(`Cashback wallet updated: ₹${currentEarned} → ₹${newEarned} (Added ₹${totalAmount}, Deducted ₹${cashbackToDeduct})`);
            ...
            if (currentUser && currentUser.uid === userId) {
                ...
                // Show user notification about cashback credit/deduction
                setTimeout(() => {
                    let notificationMsg = `🎉 Great news! ${formatCurrency(totalAmount)} has been added to your cashback wallet!`;
                    if (cashbackToDeduct > 0) {
                        notificationMsg = `🎉 Refund processed! ${formatCurrency(totalAmount)} added to wallet, and ${formatCurrency(cashbackToDeduct)} previously earned cashback was deducted.`;
                    }
        ...
        let adminMsg = `Refund completed! ${formatCurrency(totalAmount)} added to customer's cashback wallet ✅`;
        if (cashbackToDeduct > 0) adminMsg += ` (${formatCurrency(cashbackToDeduct)} cashback deducted)`;
        showNotification(adminMsg, 'success');
            ...
            refundElement.innerHTML = `
                ...
                <div style="display: flex; gap: 10px; margin-bottom: 15px;">
                    ...
                    <div style="flex: 1; background: #e8f4fd; padding: 10px; border-radius: 6px; text-align: center; display: flex; flex-direction: column; justify-content: center; border: 2px solid #17a2b8;">
                        ...
                        <div style="font-size: 18px; font-weight: 700; color: #17a2b8;">${formatCurrency(order.totalAmount)}</div>
...
window.addSlide = async function () {
    ...
    const linkedCategory = document.getElementById('slideCategoryLink')?.value || '';
    ...
    const success = await addSlideToFirebase(title, url, linkedCategory);
    if (success) {
        loadSlidesData();
        // Clear form
        document.getElementById('slideTitle').value = '';
        document.getElementById('slideUrl').value = '';
        const catSelect = document.getElementById('slideCategoryLink');
...
// Open inline edit form for a slide
window.editSlide = function (slideId) {
    ...
    const form = document.getElementById(`edit-form-${slideId}`);
    if (!form) return;
    form.style.display = 'block';

    // Populate the category dropdown for this edit form
    const select = document.getElementById(`editCat-${slideId}`);
    if (select && categoriesData) {
        select.innerHTML = '<option value="">-- No Category Link --</option>';
        categoriesData.forEach(cat => {
            ...
            if (cat.subcategories && cat.subcategories.length > 0) {
                cat.subcategories.forEach(sub => {
                    ...
                    subOpt.value = `${cat.name}::${sub.name}`;
                    subOpt.textContent = `  └ ${sub.name}`;
                    select.appendChild(subOpt);
                });
            }
        });
        // Pre-select existing linked category
        const slide = slidesData.find(s => s.id === slideId);
        if (slide && slide.linkedCategory) select.value = slide.linkedCategory;
    }
};

// Save edits to a slide
window.saveSlideEdit = async function (slideId) {
    const title = document.getElementById(`editTitle-${slideId}`)?.value?.trim();
    const url = document.getElementById(`editUrl-${slideId}`)?.value?.trim();
    const linkedCategory = document.getElementById(`editCat-${slideId}`)?.value || '';

    if (!title || !url) {
        showNotification('Title and URL are required', 'warning');
        return;
    }

    try {
        const convertedUrl = convertGoogleDriveUrl(url);
        const updateData = { title, url: convertedUrl, originalUrl: url };
        if (linkedCategory) {
            updateData.linkedCategory = linkedCategory;
        } else {
            // Remove the field if no category is selected
            const { deleteField } = await import('https://www.gstatic.com/firebasejs/11.4.0/firebase-firestore.js');
            updateData.linkedCategory = deleteField();
        }
        await updateDoc(doc(db, 'slides', slideId), updateData);
        showNotification('Slide updated successfully! ✅', 'success');
        await loadSlidesFromFirebase();
        loadSlidesData();
    } catch (error) {
        console.error('Error updating slide:', error);
        showNotification('Error updating slide', 'error');
    }
};
...
function loadSlidesData() {
    ...
    slidesData.forEach((slide, index) => {
        ...
        slideItem.id = `slide-item-${slide.id}`;
        const linkedLabel = slide.linkedCategory
            ? `<small style="color: #27ae60; font-weight: 600;">🔗 Linked: ${slide.linkedCategory}</small>`
            : `<small style="color: #aaa;">No category linked</small>`;
        slideItem.innerHTML = `
            ...
            <div class="action-buttons">
                <button class="btn-edit" onclick="editSlide('${slide.id}')" style="background:#3498db;color:white;border:none;padding:6px 12px;border-radius:5px;cursor:pointer;font-size:12px;margin-right:4px;">Edit</button>
                <button class="btn-danger" onclick="removeSlide('${slide.id}')">Remove</button>
            </div>
            <!-- Inline edit form (hidden by default) -->
            <div id="edit-form-${slide.id}" style="display:none; margin-top:12px; padding:12px; background:#f0f4f8; border-radius:8px; border:1px solid #bcd;">
                <input type="text" id="editTitle-${slide.id}" value="${slide.title.replace(/"/g, '&quot;')}" placeholder="Slide Title" style="width:100%;padding:8px;margin-bottom:8px;border:1px solid #ddd;border-radius:5px;box-sizing:border-box;">
                <input type="url" id="editUrl-${slide.id}" value="${(slide.originalUrl || slide.url).replace(/"/g, '&quot;')}" placeholder="Image URL" style="width:100%;padding:8px;margin-bottom:8px;border:1px solid #ddd;border-radius:5px;box-sizing:border-box;">
                <select id="editCat-${slide.id}" style="width:100%;padding:8px;margin-bottom:10px;border:1px solid #ddd;border-radius:5px;">
                    <option value="">-- No Category Link --</option>
                </select>
                <div style="display:flex;gap:8px;">
                    <button onclick="saveSlideEdit('${slide.id}')" style="flex:1;background:#27ae60;color:white;border:none;padding:8px;border-radius:5px;cursor:pointer;font-weight:600;">Save</button>
                    <button onclick="document.getElementById('edit-form-${slide.id}').style.display='none'" style="flex:1;background:#aaa;color:white;border:none;padding:8px;border-radius:5px;cursor:pointer;">Cancel</button>
                </div>
            </div>
        `;
    ...
    if (templateSelect && templatePreview) {
        templateSelect.addEventListener('change', function () {
            ...
            const previews = {
                ...
                'template-4': '<strong>💎 Compact Pro:</strong> Space-efficient premium cards with discount badges',
                'template-5': '<strong>🪄 Wide Showcase:</strong> Centered white box with responsive two-row grid (no horizontal scroll)'
            };
            templatePreview.innerHTML = previews[selectedTemplate] || previews['template-4'];
        });
    }
});
        ...
        filteredProducts.forEach(product => {
            ...
            productItem.innerHTML = `
                ...
                <div style="flex: 1;">
                    ...
                    <div style="font-size: 10px; color: #666;">${formatCurrency(product.price)} | ${product.category}</div>
                </div>
            `;
    ...
    filteredProducts.forEach(product => {
        ...
        productCard.innerHTML = `
            ...
            <div style="padding: 12px;">
                ...
                <div style="font-size: 13px; color: #059669; font-weight: bold; margin-bottom: 2px;">${formatCurrency(product.price)}</div>
...
function updateCarouselCardStates(container) {
    if (!container || !container.classList.contains('template-1')) return;
    ...
    cards.forEach((card, index) => {
        card.classList.remove('center-card', 'side-card', 'behind-card');
...
function setupTemplateOneCarousel(container) {
    if (!container || !container.classList.contains('template-1') || container.dataset.carouselBound === 'true') {
        if (container && container.classList.contains('template-1')) {
            updateCarouselCardStates(container);
        }
        return;
    }
    ...
    const resizeObserver = new ResizeObserver(() => updateCarouselCardStates(container));
    resizeObserver.observe(container);

    container.dataset.carouselBound = 'true';
    container.carouselResizeObserver = resizeObserver;
    ...
    homeEvents.forEach(event => {
        ...
        if (event.type === 'category' && event.category) {
            ...
            validProducts.forEach(product => {
                const productCard = createEventProductCard(product);
...
function getEventProductDiscountPercent(product) {
    const explicitDiscount = Number(product && product.discount);
    if (Number.isFinite(explicitDiscount) && explicitDiscount > 0) return Math.round(explicitDiscount);
    const originalPrice = Number(product && product.originalPrice);
    const price = Number(product && product.price);
    if (!Number.isFinite(originalPrice) || !Number.isFinite(price) || originalPrice <= 0) return 0;
    if (originalPrice <= price) return 0;
    return Math.round(((originalPrice - price) / originalPrice) * 100);
}

function createEventProductCard(product) {
    const card = document.createElement('div');
    card.className = 'event-product-card';
    card.setAttribute('data-product-id', product.id);
    card.onclick = () => openProductModal(product.id);

    const discountPercent = getEventProductDiscountPercent(product);
    const discountText = discountPercent > 0 ? `Min. ${discountPercent}% Off` : '';

    const imgUrl = product.imageUrl || product.image || '';
    card.innerHTML = `
        <div class="event-product-image-wrap">
            <img class="event-product-image" src="${imgUrl}" alt="${product.name || ''}" loading="lazy" onerror="this.style.display='none'">
        </div>
        ${discountText ? `<div class="event-product-off">${discountText}</div>` : ''}
        <div class="event-product-name">${product.name || ''}</div>
    `;
...
window.toggleAdmin = async function (email) {
    ...
    try {
        ...
        // Reload users to reflect changes (maintain search if active)
        const userSearch = document.getElementById('userSearchInput');
        loadUsersData(userSearch ? userSearch.value : '');
...
window.loadUsersData = async function (searchQuery = '') {
    ...
    try {
        ...
        usersSnapshot.forEach((doc) => {
            ...
            // Check if we already have this user (deduplicate by email)
            const existingIndex = allUsers.findIndex(u => u.email === email);

            // Get cashback values - prefer new fields, fall back to old walletBalance
            let earnedCashback = parseFloat(userData.earnedCashback) || 0;
            let pendingCashback = parseFloat(userData.pendingCashback) || 0;
            
            // Log raw data for debugging
            console.log(`📊 User ${email} raw data:`, {
                earnedCashback: userData.earnedCashback,
                pendingCashback: userData.pendingCashback,
                walletBalance: userData.walletBalance,
                finalEarned: earnedCashback,
                finalPending: pendingCashback
            });
            
            // If new fields don't exist but old walletBalance does, use it for display
            if (earnedCashback === 0 && pendingCashback === 0 && userData.walletBalance) {
                earnedCashback = parseFloat(userData.walletBalance) || 0;
                console.log(`🔄 Using old walletBalance for ${email}: ₹${earnedCashback}`);
                // Migrate to new structure in background
                setDoc(doc(db, 'users', doc.id), {
                    earnedCashback: earnedCashback,
                    pendingCashback: 0
                }, { merge: true }).then(() => {
                    console.log(`✅ Migrated walletBalance to earnedCashback for ${email}: ₹${earnedCashback}`);
                }).catch(err => {
            ...
            const userObj = {
                ...
                isAdmin: (email === 'moghaeashu@gmail.com') || (userData.isAdmin === true),
            ...
            if (existingIndex !== -1) {
                // If duplicate, keep the one with better data or more recent (optional)
        ...
        if (query) {
            filteredUsers = allUsers.filter(user => {
                ...
                const phoneStr = (user.phone || '').toString().toLowerCase();
        ...
        if (userCountDisplay) {
            userCountDisplay.textContent = `${filteredUsers.length} Users${query ? ' Found' : ' Total'}`;
        }

        if (filteredUsers.length === 0) {
            usersList.innerHTML = `<div style="text-align: center; padding: 30px; color: #666;">
                ${query ? '🔍 No users found matching your search' : '👥 No registered users found yet'}
            </div>`;
            return;
        }

        // Improved Sorting: Relevance first (starts with query), then Date (Newest first)
        filteredUsers.sort((a, b) => {
            if (query) {
                ...
                const aStarts = aName.startsWith(query) || aEmail.startsWith(query);
                const bStarts = bName.startsWith(query) || bEmail.startsWith(query);
        ...
        // Clear usersList before appending
        usersList.innerHTML = '';
        usersList.innerHTML = '';
        ...
        filteredUsers.forEach(user => {
            ...
            // Parse and validate cashback values
            const earnedCashback = parseFloat(user.earnedCashback) || 0;
            const pendingCashback = parseFloat(user.pendingCashback) || 0;
            
            console.log(`👤 Admin panel - Loading user ${user.email}:`, {
                earned: earnedCashback,
                pending: pendingCashback,
                raw: {
                    earnedCashback: user.earnedCashback,
            ...
            userItem.innerHTML = `
                <div class="user-info">
                    ...
                    <div class="user-wallet-admin" style="margin-top: 8px; padding: 8px; background: #f0f9ff; border-radius: 6px; border-left: 4px solid #0ea5e9; font-size: 11px;">
                        <div style="font-weight: 700; color: #0369a1; margin-bottom: 4px;">💰 CASHBACK WALLET</div>
                        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 8px;">
                            <div>
                                <span style="color: #059669; font-weight: 600;">✅ Earned:</span> 
                                <b style="color: #059669;">${formatCurrency(earnedCashback)}</b>
                            </div>
                            <div>
                                <span style="color: #d97706; font-weight: 600;">⏳ Pending:</span> 
                                <b style="color: #d97706;">${formatCurrency(pendingCashback)}</b>
                            </div>
                            <button onclick="editUserWallet('${user.uid}', '${user.email}', ${earnedCashback}, ${pendingCashback})" 
                                    style="padding: 4px 10px; font-size: 10px; cursor: pointer; background: #0ea5e9; color: white; border: none; border-radius: 4px; font-weight: 600; transition: all 0.2s ease;"
...
// Edit User Wallet Balance (Admin)
window.editUserWallet = async function (uid, email, currentEarned, currentPending) {
    console.log('📝 editUserWallet called:', { uid, email, currentEarned, currentPending });
    
    const newEarned = prompt(`Update Earned Cashback for ${email}:\nCurrent: ₹${formatCurrency(currentEarned)}`, currentEarned);
    if (newEarned === null) return;

    const newPending = prompt(`Update Pending Cashback for ${email}:\nCurrent: ₹${formatCurrency(currentPending)}`, currentPending);
    if (newPending === null) return;

    try {
        const earnedVal = parseFloat(newEarned);
        const pendingVal = parseFloat(newPending);

        console.log('💰 Validating new values:', { earnedVal, pendingVal });

        if (isNaN(earnedVal) || isNaN(pendingVal)) {
            showNotification('❌ Please enter valid numbers', 'error');
            return;
        }

        if (earnedVal < 0 || pendingVal < 0) {
            showNotification('❌ Cashback values cannot be negative', 'error');
            return;
        }

        console.log('📡 Updating Firestore with new values...');
        
        // Update Firestore
        await updateDoc(doc(db, 'users', uid), {
            earnedCashback: Math.round(earnedVal * 100) / 100,
            pendingCashback: Math.round(pendingVal * 100) / 100
        });

        console.log('✅ Wallet balance updated successfully');
        showNotification('✅ Wallet balance updated successfully! 🪙', 'success');

        // Wait a moment for Firestore to propagate, then reload list
        setTimeout(() => {
            const userSearch = document.getElementById('userSearchInput');
            loadUsersData(userSearch ? userSearch.value : '');
            console.log('🔄 Reloaded user list after wallet update');
        }, 500);

        // If editing current user's wallet, refresh their display
        if (currentUser && currentUser.uid === uid) {
            console.log('🔄 Refreshing current user wallet display');
            loadWalletBalance();
        }
...
function loadAdminData() {
    loadSlidesData();
    // loadUsersData(); // Removed to prevent double loading, users load when tab is clicked
...
// Open a category or subcategory linked to a banner slide (called on slide click)
window.openLinkedCategory = function (linkedValue) {
    if (!linkedValue) return;
    // Format: "CategoryName" or "CategoryName::SubcategoryName"
    if (linkedValue.includes('::')) {
        const [catName, subName] = linkedValue.split('::');
        openCategoryModal(catName.trim(), subName.trim());
    } else {
        openCategoryModal(linkedValue.trim());
    }
};

// Populate the slide category dropdown from loaded categories/subcategories
function populateSlideCategoryDropdown() {
    const select = document.getElementById('slideCategoryLink');
    if (!select || !categoriesData) return;
    ...
    categoriesData.forEach(cat => {
        ...
        if (cat.subcategories && cat.subcategories.length > 0) {
            cat.subcategories.forEach(sub => {
                ...
                subOption.value = `${cat.name}::${sub.name}`;
...
function updateSlideshow() {
    ...
    slidesData.forEach((slide, index) => {
        ...
        const clickAttr = slide.linkedCategory
            ? `onclick="openLinkedCategory('${slide.linkedCategory.replace(/'/g, "\\'")}')"`
            : '';
        const cursorStyle = slide.linkedCategory ? 'cursor: pointer;' : '';

        slideDiv.innerHTML = `
            <img src="${slide.url}" 
                 alt="${slide.title}" 
                 style="width: 100%; height: 100%; object-fit: cover; ${cursorStyle}"
                 ${clickAttr}
    ...
    // Enable manual swipe/drag navigation
    setupSlideshowSwipe(slideshowContainer);
...
// Track last category modal that opened search, so back can return correctly
let lastCategorySearchContext = null;
...
window.addEventListener('popstate', function (event) {
    ...
    switch (lastNavigation.type) {
        ...
        case 'section':
            ...
            // Switch main section without adding new history
...
function isWebView() {
    ...
    const webViewIndicators = [
        'wv', // WebView indicator in Android
        'webview', // General WebView
        'kodular', // Kodular specific
        'app inventor', // MIT App Inventor (Kodular is based on this)
        'thunkable', // Similar platforms
        'appgyver', // Similar platforms
        'adobe air', // Some WebView frameworks
        'crosswalk', // Crosswalk WebView
        'chromium', // Sometimes indicates WebView
        '; wv)' // Android WebView pattern
    ];
    
    // Check if any WebView indicator is in user agent
    const hasWebViewIndicator = webViewIndicators.some(indicator => 
        userAgent.includes(indicator)
    );
    
    // iOS WebView detection
    const isIOSWebView = isIOS && !standalone && !/safari/.test(userAgent);
    ...
    return isIOSWebView || isAndroidWebView || hasWebViewIndicator;
}

// Hide Play Store image if running in WebView
function handlePlayStoreVisibility() {
    const playStoreElement = document.querySelector('.floating-playstore');
    if (playStoreElement) {
        if (isWebView()) {
            playStoreElement.style.display = 'none';
            console.log('📱 WebView detected - Play Store image hidden');
        } else {
            playStoreElement.style.display = 'flex';
            console.log('🌐 Browser detected - Play Store image visible');
        }
    }
}
...
document.addEventListener('DOMContentLoaded', async function () {
    // Handle Play Store image visibility based on WebView detection
    handlePlayStoreVisibility();
...
window.showInvoice = async function (orderId) {
    ...
    // Show print button only to admin
    if (currentUser) {
...
window.openWalletModal = function () {
    ...
    // Also check for any pending cashback that should be earned
    setTimeout(checkPendingCashbackStatus, 1000);
    ...
    if (currentUser) {
        getDoc(doc(db, 'users', currentUser.uid)).then(userDoc => {
            if (userDoc.exists()) {
                ...
                console.log('🔍 Debug - Current user wallet data:', {
                    earnedCashback: data.earnedCashback,
...
async function loadWalletBalance() {
    ...
    try {
        ...
        if (userDoc.exists()) {
            ...
            // Log raw data for debugging
            console.log(`📊 Current user raw data:`, {
                earnedCashback: userData.earnedCashback,
                pendingCashback: userData.pendingCashback,
                walletBalance: userData.walletBalance,
                finalEarned: earnedCashback,
            ...
            if (earnedCashback === 0 && pendingCashback === 0 && userData.walletBalance) {
                earnedCashback = parseFloat(userData.walletBalance) || 0;
                console.log(`🔄 Using old walletBalance for current user: ₹${earnedCashback}`);
                // Migrate to new structure in background
                setDoc(userRef, {
                    earnedCashback: earnedCashback,
                    pendingCashback: 0
                }, { merge: true }).then(() => {
                    console.log(`✅ Migrated walletBalance to earnedCashback for current user: ₹${earnedCashback}`);
                }).catch(err => {
                    console.error('❌ Migration failed for current user:', err);
                });
            }

            console.log('💰 Loaded cashback data:', {
                earned: earnedCashback,
                pending: pendingCashback,
                raw: {
                    earnedCashback: userData.earnedCashback,
            ...
            if (earnedElement) {
                ...
                earnedElement.textContent = formatCurrency(roundedEarned);
                console.log('✅ Updated earned cashback element:', formatCurrency(roundedEarned));
            } else {
                console.warn('⚠️ earnedCashback element not found');
            }
            ...
            if (pendingElement) {
                ...
                pendingElement.textContent = formatCurrency(roundedPending);
                console.log('✅ Updated pending cashback element:', formatCurrency(roundedPending));
            } else {
                console.warn('⚠️ pendingCashback element not found');
            }

            console.log('✅ Wallet cashback successfully updated in UI');
        } else {
            console.log('📝 No user document found, initializing with 0 values');
            // Initialize with 0 values
            const earnedElement = document.getElementById('earnedCashback');
            const pendingElement = document.getElementById('pendingCashback');
            if (earnedElement) earnedElement.textContent = formatCurrency(0);
            if (pendingElement) pendingElement.textContent = formatCurrency(0);
        }
    } catch (error) {
...
// Check for pending cashback that should be earned
async function checkPendingCashbackStatus() {
    if (!currentUser) return;
    ...
    try {
        ...
        if (userDoc.exists()) {
            const userData = userDoc.data();
            const pendingCashback = parseFloat(userData.pendingCashback) || 0;
            
            if (pendingCashback > 0) {
                // Check if there are any completed orders with scheduled cashback
                const ordersQuery = query(
                    collection(db, 'orders'),
                    where('userId', '==', currentUser.uid),
                    where('status', '==', 'Completed'),
                    where('cashbackStatus', '==', 'scheduled')
                );
                ...
                for (const orderDoc of ordersSnapshot.docs) {
                    const orderData = orderDoc.data();
                    const completedAt = orderData.completedAt?.toDate();
                    const cashbackScheduledAt = orderData.cashbackScheduledAt?.toDate();
                    
                    // Check if 24 hours has passed since completion
                    if (completedAt && cashbackScheduledAt) {
                        const now = new Date();
                        const timeSinceScheduled = now - cashbackScheduledAt;
                        
                        if (timeSinceScheduled >= 86400000) { // 24 hours
                            console.log(`⏰ Auto-processing scheduled cashback for order ${orderDoc.id}`);
                            ...
                            // Transfer cashback
                            await completeCashbackForOrder(orderDoc.id, orderData.cashbackAmount, currentUser.uid);
                            
                            // Show notification
                            showNotification(`🎉 Your cashback of ₹${formatCurrency(orderData.cashbackAmount)} is now available!`, 'success');
        ...
        console.error('Error checking pending cashback status:', error);
    }
}
...
async function collectCashback() {
    ...
    try {
        ...
        // Show success notification
        setTimeout(() => {
            showNotification(`${formatCurrency(amount)} cashback collected! 💰`, 'success', 3000);
            closeCashbackReward();
        }, 1500);
...
async function completeCashbackForOrder(orderId, cashbackAmount, customerUserId = null) {
    console.log('🔄 completeCashbackForOrder called:', { orderId, cashbackAmount, customerUserId });
    
    if (!cashbackAmount || cashbackAmount <= 0) {
        console.log('❌ Invalid cashback amount:', cashbackAmount);
        return;
    }
    ...
    try {
        ...
        console.log('📡 Processing cashback completion for user:', userId);
        const userRef = doc(db, 'users', userId);
        const userDoc = await getDoc(userRef);

        if (userDoc.exists()) {
            const userData = userDoc.data();
            const currentPending = parseFloat(userData.pendingCashback) || 0;
            const currentEarned = parseFloat(userData.earnedCashback) || 0;

            console.log('💰 Current cashback status:', {
                pending: currentPending,
                earned: currentEarned,
                toTransfer: cashbackAmount
            });

            // Validate that there's enough pending cashback to transfer
            if (currentPending < cashbackAmount) {
                console.warn('⚠️ Pending cashback insufficient:', currentPending, '<', cashbackAmount);
                // Still proceed with available amount
                cashbackAmount = currentPending;
            }
            ...
            console.log('📊 New cashback values:', {
                newPending,
                newEarned,
                transferred: cashbackAmount
            });
            ...
            console.log('✅ Cashback transferred successfully:', {
                orderId,
                transferred: cashbackAmount,
                newPending,
                newEarned
            ...
            // Refresh wallet display for the user if they're online
            if (currentUser && currentUser.uid === userId) {
                loadWalletBalance();
                console.log('🔄 Refreshed wallet display for user after cashback transfer');
            }
        } else {
...
async function cancelCashbackForOrder(orderId, cashbackAmount, customerUserId = null) {
    console.log('🔄 cancelCashbackForOrder called:', { orderId, cashbackAmount, customerUserId });
    
    if (!cashbackAmount || cashbackAmount <= 0) {
        console.log('❌ Invalid cashback amount for cancellation:', cashbackAmount);
        return;
    }
    ...
    try {
        ...
        console.log('📡 Processing cashback cancellation for user:', userId);
        const userRef = doc(db, 'users', userId);
        const userDoc = await getDoc(userRef);
        ...
        if (userDoc.exists()) {
            ...
            console.log('💰 Current pending cashback before cancellation:', currentPending);

            // Validate that there's enough pending cashback to cancel
            if (currentPending < cashbackAmount) {
                console.warn('⚠️ Pending cashback insufficient for cancellation:', currentPending, '<', cashbackAmount);
                // Still proceed with available amount
                cashbackAmount = currentPending;
            }
            ...
            console.log('📊 New pending cashback after cancellation:', {
                oldPending: currentPending,
                newPending,
                cancelled: cashbackAmount
            });
            ...
            if (currentUser && userId === currentUser.uid) {
                console.log('🔄 Refreshing UI for current user after cancellation');
                loadWalletBalance(); // Refresh UI
            }
        } else {
            console.error('❌ User document not found for cancellation:', userId);
        }
    } catch (error) {
...
window.shareProduct = async function(productId) {
    try {
        ...
        // Create product share URL with query parameter
        const shareUrl = `${currentUrl}?product=${productId}`;
        
        // Check if Web Share API is available
        if (navigator.share) {
            await navigator.share({
                title: 'Check out this product on Ficokart!',
                text: 'I found this amazing product on Ficokart. Check it out!',
                url: shareUrl
            });
            ...
            const confirmCopy = confirm('Product link copied to clipboard!\n\nLink: ' + shareUrl + '\n\nClick OK to continue');
        }
    } catch (error) {
        console.error('Error sharing product:', error);
        showNotification('Error sharing product. Please try again.', 'error');
    }
};
...
window.applyWalletBalance = async function (orderAmount) {
    ...
    try {
        ...
        if (earnedCashback <= 0) {
            // Check if user has pending cashback
            const pendingCashback = userDoc.exists() ? (userDoc.data().pendingCashback || 0) : 0;
            if (pendingCashback > 0) {
                showNotification(`⏳ You have ₹${formatCurrency(pendingCashback)} pending cashback. It will be available after your order is completed!`, 'warning');
            } else {
                showNotification('No cashback available to use', 'warning');
            }
            return;
        }
        ...
        showNotification(`${formatCurrency(amountToApply)} wallet balance applied!`, 'success');
...
window.removeWalletBalance = function () {
    if (appliedWalletAmount > 0) {
        ...
        showNotification(`${formatCurrency(removedAmount)} wallet balance removed`, 'info');
    }
};
...
function showWalletAppliedInfo(amount) {
    ...
    if (appliedInfo && appliedAmountSpan && applyBtn) {
        appliedAmountSpan.textContent = formatCurrency(amount);
...
async function loadWalletBalanceInModal() {
    console.log('🔄 loadWalletBalanceInModal called');
    if (!currentUser) {
        console.log('❌ No currentUser in loadWalletBalanceInModal');
        return;
    }

    try {
        console.log('📡 Fetching user data for modal wallet:', currentUser.uid);
        const userRef = doc(db, 'users', currentUser.uid);
        const userDoc = await getDoc(userRef);
        ...
        if (userDoc.exists()) {
            const userData = userDoc.data();
            // Get cashback value - prefer new field, fall back to old walletBalance
            earnedCashback = parseFloat(userData.earnedCashback) || 0;
            
            // Log raw data for debugging
            console.log(`📊 Modal wallet raw data:`, {
                earnedCashback: userData.earnedCashback,
                walletBalance: userData.walletBalance,
                finalEarned: earnedCashback
            });
            
            // If new field doesn't exist but old walletBalance does, use it for display
            if (earnedCashback === 0 && userData.walletBalance) {
                earnedCashback = parseFloat(userData.walletBalance) || 0;
                console.log(`🔄 Using old walletBalance for modal: ₹${earnedCashback}`);
                // Migrate to new structure in background
                setDoc(userRef, {
                    earnedCashback: earnedCashback
                }, { merge: true }).then(() => {
                    console.log(`✅ Migrated walletBalance to earnedCashback for modal: ₹${earnedCashback}`);
                }).catch(err => {
                    console.error('❌ Modal wallet migration failed:', err);
                });
            }
            
            console.log('💰 Modal wallet - Loaded earned cashback:', earnedCashback);
        } else {
            console.log('📝 Modal wallet - No user document found, using 0');
        }

        const balanceElement = document.getElementById('availableWalletBalance');
        if (balanceElement) {
            const roundedEarned = Math.round(earnedCashback * 100) / 100;
            balanceElement.textContent = formatCurrency(roundedEarned);
            console.log('✅ Modal wallet - Updated balance element:', formatCurrency(roundedEarned));
        } else {
            console.warn('⚠️ Modal wallet - availableWalletBalance element not found');
        }

        console.log('✅ Modal wallet balance loaded successfully');
    } catch (error) {
        console.error('❌ Error loading earned cashback balance in modal:', error);
...
window.printInvoice = function () {
    ...
    // Create a hidden iframe for printing to avoid mobile browser reload issues
    ...
    iframeDoc.write(`
        ...
        <head>
            <title>Invoice</title>
            <style>
                body { padding: 10px; font-family: Arial, sans-serif; margin: 0; font-size: 16px; }
                * { max-width: 100%; box-sizing: border-box; }
                @media print {
                    ...
                    #invoiceContent, body > div {
                        transform-origin: top left;
                        width: 100% !important;
                        max-width: 100% !important;
                    }
                    /* Prevent page breaks inside invoice */
                    table, tr, td, th, div, p, h1, h2, h3 {
    ...
    // Cleanup iframe after a delay to allow print dialog on mobile
    setTimeout(() => {
        if (document.body.contains(iframe)) {
            document.body.removeChild(iframe);
        }
    }, 10000); // 10 seconds is usually enough for the print dialog to spawn
};
...
function generateInvoiceHTML(order) {
    ...
    if (orderItems.length === 0) {
        ...
        if (order.productName || order.name) {
            const singleItem = {
                ...
                selectedColor: order.selectedColor || order.color || null
            };
    ...
    orderItems.forEach(item => {
        ...
        const productImage = item.imageUrl || item.image || item.productImage;

        if (productImage && productImage !== 'null' && productImage !== '') {
            productImageHTML = `<img class="product-image" src="${productImage}" alt="Product" style="width: 30px; height: 30px; border-radius: 3px; object-fit: cover; margin-right: 8px;" onerror="this.style.display='none'">`;
        }
        ...
        if (originalPrice > itemPrice && discount > 0) {
            priceDisplay = `
                <div style="text-align: center;">
                    ...
                    <div style="color: #999; font-size: 11px; text-decoration: line-through; margin-bottom: 2px;">${formatCurrency(originalPrice)}</div>
                    <div style="font-size: 11px; color: #27ae60; font-weight: 600; margin-bottom: 1px;">Special Price</div>
                    <div style="color: #27ae60; font-weight: 700;">${formatCurrency(itemPrice)}</div>
            ...
            priceDisplay = formatCurrency(itemPrice);
        }

        itemsHTML += `
            <tr>
                <td style="padding: 6px; border-bottom: 1px solid #eee; text-align: left; font-size: 12px; vertical-align: top;">
                    <div style="display: flex; align-items: flex-start;">
                        ${productImageHTML}
                        <div style="flex: 1;">
                            <div class="product-name" style="font-weight: 600; color: #2c3e50; line-height: 1.3;">${item.name || item.productName || 'Product'}</div>
                            <div class="product-details" style="color: #666; line-height: 1.2;">
                                ${item.productSize || item.selectedSize ? `S: ${item.productSize || item.selectedSize}` : ''}
                                ${(item.productSize || item.selectedSize) && (item.selectedColor || item.color) ? ' | ' : ''}
                                ${item.selectedColor || item.color ? `C: ${item.selectedColor || item.color}` : ''}
                            </div>
                            ${originalPrice > itemPrice && discount > 0 ? `<div style="color: #27ae60; font-size: 10px; font-weight: 600;">💰 Saved: ${formatCurrency((originalPrice - itemPrice) * itemQuantity)}</div>` : ''}
                        </div>
                    </div>
                </td>
                <td style="padding: 6px; border-bottom: 1px solid #eee; text-align: center; font-size: 12px;">${formatCurrency(itemPrice)}</td>
                <td style="padding: 6px; border-bottom: 1px solid #eee; text-align: center; font-size: 12px;">${itemQuantity}</td>
                <td style="padding: 6px; border-bottom: 1px solid #eee; text-align: right; font-weight: 600; font-size: 12px; color: #27ae60;">${formatCurrency(itemTotal)}</td>
            </tr>
        `;
    });
    ...
    return `
        ...
        <head>
            ...
            <style>
                ...
                @media print {
                    ...
                    /* Force compact layout for many items */
                    .items-table td {
        ...
        <body>
            <div class="invoice-container" style="background: white; padding: 20px; max-width: 600px; margin: 0 auto;">
                <!-- Header with Logo -->
                <div style="text-align: center; margin-bottom: 15px; border-bottom: 2px solid #3498db; padding-bottom: 10px;">
                    <div style="display: flex; align-items: center; justify-content: center; gap: 12px;">
                        <img src="ficokartstamp.png" alt="Ficokart Stamp" style="width: 80px; height: 80px; object-fit: contain;" onerror="this.style.display='none'">
                        <div style="text-align: center;">
                            <div style="font-size: 28px; font-weight: 800; color: #000; letter-spacing: 1px;">Ficokart</div>
                            <h1 style="color: #2c3e50; margin: 0; font-size: 24px; font-weight: 700;">INVOICE BILL</h1>
                            <p style="color: #7f8c8d; margin: 3px 0 0 0; font-size: 14px;">Ficokart E-commerce</p>
                        </div>
                    </div>
                </div>
            ...
            <div style="display: flex; justify-content: space-between; margin-bottom: 15px; flex-wrap: wrap;">
                <div style="flex: 1; width: 90vw;">
                    <h3 style="color: #2c3e50; margin: 0 0 5px 0; font-size: 16px;">Invoice Details</h3>
                    <p style="margin: 2px 0; color: #555; font-size: 13px;"><strong>Invoice #:</strong> INV-${order.id.substring(0, 8).toUpperCase()}</p>
                    <p style="margin: 2px 0; color: #555; font-size: 13px;"><strong>Order ID:</strong> ${order.id}</p>
                    <p style="margin: 2px 0; color: #555; font-size: 13px;"><strong>Date:</strong> ${orderDate}</p>
                </div>
                
                <div style="flex: 1; width: 90vw; text-align: right;">
                    <h3 style="color: #2c3e50; margin: 0 0 5px 0; font-size: 16px;">Customer Details</h3>
                    <p style="margin: 2px 0; color: #555; font-size: 13px;"><strong>Name:</strong> ${order.userName || 'N/A'}</p>
                    <p style="margin: 2px 0; color: #555; font-size: 13px;"><strong>Email:</strong> ${order.userEmail || 'N/A'}</p>
                    <p style="margin: 2px 0; color: #555; font-size: 13px;"><strong>Phone:</strong> ${order.userPhone || 'N/A'}</p>
                    <p style="margin: 2px 0; color: #555; font-size: 13px;"><strong>Payment:</strong> ${(() => {
            const method = order.paymentMethod || 'Not specified';
            if (method === 'cod') return 'Cash on Delivery (COD)';
            ...
            <!-- Address (compact) -->
            <div style="margin-bottom: 15px;">
                <p style="margin: 2px 0; color: #555; font-size: 12px;"><strong>Delivery Address:</strong> ${order.userAddress || 'N/A'}</p>
            </div>
            
            <!-- Items Table -->
            <div style="margin-bottom: 15px;">
                <h3 style="color: #2c3e50; margin: 0 0 8px 0; font-size: 14px;">Order Items (${orderItems.length})</h3>
                <table class="items-table" style="width: 100%; border-collapse: collapse; border: 1px solid #ddd; border-radius: 4px; overflow: hidden;">
                    <thead>
                        <tr style="background: #f8f9fa;">
                            <th style="padding: 8px; text-align: left; color: #2c3e50; font-weight: 600; border-bottom: 1px solid #ddd; font-size: 12px;">Item</th>
                            <th style="padding: 8px; text-align: center; color: #2c3e50; font-weight: 600; border-bottom: 1px solid #ddd; font-size: 12px;">Price</th>
                            <th style="padding: 8px; text-align: center; color: #2c3e50; font-weight: 600; border-bottom: 1px solid #ddd; font-size: 12px;">Qty</th>
                            <th style="padding: 8px; text-align: right; color: #2c3e50; font-weight: 600; border-bottom: 1px solid #ddd; font-size: 12px;">Total</th>
                        </tr>
                    </thead>
                    <tbody>
                        ${itemsHTML}
            ...
            <!-- Total Section -->
            <div class="total-section" style="text-align: right; margin-bottom: 15px;">
                <div style="background: #f8f9fa; padding: 12px; border-radius: 4px; display: inline-block; width: 90vw;">
                    ${totalDiscount > 0 ? `
                        <div style="display: flex; justify-content: space-between; margin-bottom: 6px;">
                            <span style="color: #555; font-size: 11px;">Original Total:</span>
                            <span style="font-weight: 600; color: #999; font-size: 11px; text-decoration: line-through;">₹${originalTotal + deliveryAmount}</span>
                        </div>
                        <div style="display: flex; justify-content: space-between; margin-bottom: 6px;">
                            <span style="color: #27ae60; font-size: 11px;">Discount:</span>
                            <span style="font-weight: 600; color: #27ae60; font-size: 11px;">-₹${totalDiscount}</span>
                        </div>
                    ` : ''}
                    <div style="display: flex; justify-content: space-between; margin-bottom: 6px;">
                        <span style="color: #555; font-size: 11px;">Subtotal:</span>
                        <span style="font-weight: 600; color: #2c3e50; font-size: 11px;">₹${totalAmount}</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; margin-bottom: 6px;">
                        <span style="color: #555; font-size: 11px;">Delivery:</span>
                        <span style="font-weight: 600; color: #27ae60; font-size: 11px;">${deliveryCharges === 'FREE' ? 'FREE' : '₹' + deliveryCharges}</span>
                    </div>
                    ${totalDiscount > 0 ? `
                        <div style="display: flex; justify-content: space-between; margin-bottom: 6px; color: #27ae60;">
                            <span style="font-size: 9px; font-weight: 600;">💰 You Saved:</span>
                            <span style="font-size: 9px; font-weight: 600;">₹${totalDiscount}</span>
                        </div>
                    ` : ''}
                    ${walletUsed > 0 ? `
                        <div style="display: flex; justify-content: space-between; margin-bottom: 6px; color: #8e44ad;">
                            <span style="font-size: 11px; font-weight: 600;">💰 Wallet Applied:</span>
                            <span style="font-size: 11px; font-weight: 600;">-₹${walletUsed}</span>
                        </div>
                    ` : ''}
                    <div style="display: flex; justify-content: space-between; padding-top: 6px; border-top: 1px solid #3498db;">
                        <span style="font-size: 12px; font-weight: 700; color: #2c3e50;">Grand Total:</span>
                        <span style="font-size: 12px; font-weight: 700; color: #27ae60;">₹${finalTotal}</span>
                    </div>
                </div>
            </div>
            
            <!-- Footer -->
            <div style="text-align: center; padding-top: 10px; border-top: 1px solid #ecf0f1; color: #7f8c8d;">
                <p style="margin: 2px 0; font-size: 9px;">Thank you for shopping with Ficokart!</p>
                <p style="margin: 2px 0; font-size: 9px;">For any queries, contact us at support@ficokart.com</p>
                <p style="margin: 5px 0 0 0; font-size: 8px; color: #bdc3c7;">This is a computer generated invoice and does not require signature.</p>
            </div>
            </div>
        </body>
        </html>
...
window.openAddressMapModal = function () {
    ...
    // Show Location Accuracy prompt
    const locModal = document.getElementById('locationAccuracyModal');
    if (locModal) {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Check for shared product URL parameter
    const urlParams = new URLSearchParams(window.location.search);
    const sharedProductId = urlParams.get('product');
    ...
    if (sharedProductId) {
        ...
        // Function to try opening the shared product
        const tryOpenSharedProduct = () => {
            const product = productsData.find(p => p.id === sharedProductId);
            if (product) {
                console.log('Opening shared product:', product.name);
                window.openProductModal(sharedProductId);
        ...
        // Try immediately if products are already loaded
        if (productsData && productsData.length > 0) {
            if (!tryOpenSharedProduct()) {
                console.log('Product not found in loaded data:', sharedProductId);
                showNotification('Product not found', 'error');
                // Clear URL parameter
                window.history.replaceState({}, document.title, window.location.pathname);
            }
            ...
            const checkAndOpen = () => {
                attempts++;
                console.log(`Checking for shared product (attempt ${attempts}/${maxAttempts})`);
                
                if (productsData && productsData.length > 0) {
                    if (!tryOpenSharedProduct()) {
                        console.log('Product not found after products loaded:', sharedProductId);
                        showNotification('Product not found', 'error');
                        // Clear URL parameter
                        window.history.replaceState({}, document.title, window.location.pathname);
                    }
                    ...
                    console.log('Failed to load products after maximum attempts');
                    showNotification('Failed to load product. Please refresh the page.', 'error');
                    // Clear URL parameter
                    window.history.replaceState({}, document.title, window.location.pathname);
                }
            };
...
function updateHomeAddressDisplay() {
    ...
    const categoryNavAddressText = document.getElementById('categoryNavAddressText');
    ...
    if (!currentUser) {
        ...
        if (categoryNavAddressText) categoryNavAddressText.textContent = 'Not Set';
        return;
    }
            ...
            if (userAddresses.length > 0) {
                ...
                if (selectedAddressId) {
                    ...
                    if (selectedAddress) {
                        ...
                        if (categoryNavAddressText) categoryNavAddressText.textContent = shortAddress;
                        return;
                    }
                }
                ...
                if (categoryNavAddressText) categoryNavAddressText.textContent = 'Add Address';
            }
        } else {
...
window.initiateRazorpayPayment = function () {
    ...
    // Check if this is for all cart items (now checks for items array in combined order)
    const isCartPayment = window.allCartOrdersData && window.allCartOrdersData.items && window.allCartOrdersData.items.length > 0;
    ...
    try {
        ...
        if (orderId) {
            ...
            if (window.orderFromCart && window.orderFromCart.productId === currentOrderData.productId) {
                removeFromCart(window.orderFromCart.productId, window.orderFromCart.selectedSize);
                showNotification(`✅ Payment Successful! 🎉<br>Order ID: ${orderId}<br>Total: ${formatCurrency(currentPaymentAmount)}<br>Payment ID: ${paymentResponse.razorpay_payment_id}<br>Item removed from cart.`, 'success', 8000);
                // Clear the cart flag
                window.orderFromCart = null;
            } else {
                showNotification(`✅ Payment Successful! 🎉<br>Order ID: ${orderId}<br>Total: ${formatCurrency(currentPaymentAmount)}<br>Payment ID: ${paymentResponse.razorpay_payment_id}`, 'success', 8000);
            }
...
async function handlePaymentSuccessForAllCartItems(paymentResponse) {
    try {
        // Use the stored order data from global variable (now it's a single combined order object)
        if (!window.allCartOrdersData || !window.allCartOrdersData.items) {
            showNotification('Payment successful but order data not found. Please contact support.', 'error');
            return;
        }

        // Add payment details to the combined order data
        const updatedOrderData = {
            ...window.allCartOrdersData,
        ...
        // Save the single combined order to Firebase
        const orderId = await saveOrderToFirebase(updatedOrderData, true); // Pass flag to skip individual cashback

        if (orderId) {
            // Calculate 1% combined cashback (based on original amount before wallet deduction)
            const originalTotalAmount = window.allCartOrdersData.originalAmount;
            const combinedCashbackPerHundred = 1; // 1%
            const combinedCashbackAmount = Math.round((originalTotalAmount * combinedCashbackPerHundred) / 100 * 100) / 100;

            // Add cashback to the order
            if (combinedCashbackAmount > 0) {
                const orderRef = doc(db, 'orders', orderId);
                await setDoc(orderRef, {
                    cashbackAmount: combinedCashbackAmount,
            ...
            const walletUsed = window.allCartOrdersData.walletUsed || 0;
            const walletMessage = walletUsed > 0 ? `<br>💳 Wallet used: ${formatCurrency(walletUsed)}` : '';
            const cashbackMessage = combinedCashbackAmount > 0 ? `<br>💰 Cashback: ${formatCurrency(combinedCashbackAmount)} added to pending wallet!` : '';

            showNotification(`✅ Payment Successful! 🎉<br>${window.allCartOrdersData.itemCount} items in single order<br>Total: ${formatCurrency(currentPaymentAmount)}<br>Payment ID: ${paymentResponse.razorpay_payment_id}${walletMessage}${cashbackMessage}`, 'success', 8000);

...
async function addProductToFirebase(name, imageUrl, description, finalPrice, subImages, categories, sizes, sizePricing, deliveryCharge, rating, originalPrice, discount, weightPricing, subcategories = [], sizeDiscounts = {}, keywords = '', isReturnable = true, customPricing = []) {
    try {
        const convertedUrl = convertGoogleDriveUrl(imageUrl);
        ...
        const productData = {
            ...
            weightPricing: weightPricing || { ml: [], g: [], kg: [], pc: [] },
...
window.addProduct = async function () {
    ...
    if (await addProductToFirebase(name, imageUrl, description, finalPrice, subImages, selectedCategories, sizes, sizePricing, deliveryCharge, rating, originalPrice, discount, weightPricing, selectedSubcategories, sizeDiscounts, keywords, isReturnable, customPricing)) {
...
window.openProductModalFromCart = function (productId) {
    ...
    // Prepare all images (variant prioritized)
    const allImages = [];
    if (variantImage) {
        allImages.push(variantImage);
    }
...
function updateProductModalButtons(productId) {
    ...
    let cartBtnText = isInCartWithSize ? 'Go to Cart' : 'Add to Cart';
    let cartBtnBg = isInCartWithSize ? '#27ae60' : '#3498db';
    let cartBtnAction = isInCartWithSize ? "closeProductModal(); setActiveTab('cart');" : `addToCartWithQuantity('${productId}')`;
...
window.addToCartWithQuantity = async function (productId, selectedSize = '', quantity = 1) {
    ...
    if (product) {
        ...
        if (existingItem) {
            ...
            existingItem.imageUrl = variantImage;
            existingItem.variantDisplayName = variantName;
            existingItem.variantImageUrl = variantImage;
...
window.handleOrderNowFromCart = function (productId, selectedSize, quantity) {
    ...
    window.orderFromCart = {
        ...
        variantName: cartItem.variantDisplayName || cartItem.name || product.name,
        variantImage: cartItem.variantImageUrl || cartItem.imageUrl || product.imageUrl
    };
                ...
                <td style="padding: 8px; border-bottom: 1px solid #eee; text-align: left; font-size: 15px;">
                    <div style="display: flex; align-items: center;">
                        ...
                        <div>
                            ...
                            ${item.productSize ? `<div style="color: #666; font-size: 13px;">Size: ${item.productSize}</div>` : ''}

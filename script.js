...
import { getFirestore, collection, addDoc, getDocs, deleteDoc, doc, onSnapshot, setDoc, getDoc, updateDoc, query, where, orderBy, Timestamp } from 'https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js';
...
window.selectColor = function (productId, colorIndex) {
    ...
    if (carouselImages && thumbnailStrip && images.length > 0) {
        ...
        images.forEach((imgUrl, index) => {
            const isPrimary = index === 0 && colorIndex === -1;
            slidesHTML += `
                <div class="carousel-slide" data-index="${index}" ${isPrimary ? 'data-primary="true"' : ''}>
                    <img ${isPrimary ? `id="productPrimaryImage-${productId}" data-default-src="${product.imageUrl}"` : ''} src="${imgUrl}" alt="${product.name} - ${colorName || 'Default'}" onerror="this.src='data:image/svg+xml;base64,'">
                </div>
            `;
